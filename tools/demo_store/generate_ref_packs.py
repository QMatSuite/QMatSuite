...
def _ensure_providers_loaded():
    ...
    for eng in engines:
        try:
            importlib.import_module(f"quantumvitas.drivers.{eng}.parsers")
        except ImportError:
            pass
        ...
        for obj_type in ("output", "convergence", "dos", "bands"):
            try:
                importlib.import_module(
                    f"quantumvitas.drivers.{eng}.parsers.{obj_type}"
                )
            except ImportError:
                pass
...
def main():
    ...
    # Remove ref packs that are no longer in the map
    if not dry_run and REF_PACKS_DIR.exists():
