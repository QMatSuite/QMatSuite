...
1. **文件路径**: `src/quantumvitas/io/structure_io.py`
...
2. **文件路径**: `src/quantumvitas/calculation/importers.py`
...
4. **文件路径**: `src/quantumvitas/calculation/importers.py`
...
5. **文件路径**: `src/quantumvitas/api.py`
   - **函数名**: `QVService.import_step_from_qe_input()`
...
- 但 `QVService.import_step_from_qe_input()` 每个 step 都会尝试提取 structure，然后去重注册
...
3. **文件路径**: `src/quantumvitas/api.py`
   - **函数名**: `QVService.import_structure()`（第338行）
   - **说明**: **有"新增 structure 并返回 id"的接口**，返回 `ResolvedResource`，包含 structure 的 meta.id（ULID）

4. **文件路径**: `src/quantumvitas/api.py`
   - **函数名**: `QVService.import_step_from_qe_input()`（第4247-4291行）
...
- ✅ **QVService 有"新增 structure 并返回 id"的接口**（`import_structure()` 和 `import_step_from_qe_input()` 内部逻辑）
...
1. **文件路径**: `src/quantumvitas/api.py`
   - **函数名**: `QVService.import_step_from_qe_input()`（第4237-4245行）
...
- 💡 **建议位置**: 如果实现，应该放在 `src/quantumvitas/core/structure_utils.py` 或 `src/quantumvitas/io/structure_io.py`
...
1. **文件路径**: `src/quantumvitas/calculation/structure_steps.py`
...
2. **文件路径**: `src/quantumvitas/io/structure_io.py`
...
3. **文件路径**: `src/quantumvitas/calculation/input_runner.py`
...
4. **文件路径**: `src/quantumvitas/calculation/structure_steps.py`
...
5. **文件路径**: `src/quantumvitas/calculation/input_runner.py`
