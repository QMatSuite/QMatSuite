#!/usr/bin/env python3
"""
Extract QE parameter metadata from HTML docs with section hierarchy and metadata (schema v4).
...
Output: src/quantumvitas/data/qe_module_parameters.json (schema v4)
...
6) Output:
   ...
   - Schema version 4 with module.sections (hierarchical) and module.parameters (v3 logic)
...
def parse_module_doc(module_name: str, html_path: Path, verbose: bool = False) -> Dict[str, Any]:
    ...
    # V4: Build result with sections tree and parameters
    result = {
        "doc_url": doc_url,
        "sections": sections_tree,  # V4: hierarchical section tree
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Extract QE parameter metadata from HTML docs (schema v4 with section hierarchy)."
    )
    parser.add_argument(
        "--modules",
        nargs="*",
    ...
    sys.stdout.write(
        f"[extract_v4] Generated schema v4 for {len(results)} modules, {total_params} parameters total.\n"
    )
    sys.stdout.write(
        f"[extract_v4] Total sections (hierarchical): {total_sections}\n"
    )
    sys.stdout.write(
        f"[extract_v4] Sections with description: {sections_with_description}\n"
    )
    sys.stdout.write(
        f"[extract_v4] Sections with optionality: {sections_with_optionality}\n"
    )
    sys.stdout.write(
        f"[extract_v4] Cards with options: {cards_with_options}\n"
    )
    sys.stdout.write(
        f"[extract_v4] Parameters with indexing metadata: {params_with_indexing}\n"
    )
    sys.stdout.write(
        f"[extract_v4] Parameters with status: {params_with_status}\n"
    )
    sys.stdout.write(
        f"[extract_v4] Parameters with see_also: {params_with_see_also}\n"
    )
    sys.stdout.write(
        f"[extract_v4] Output written to {args.output}\n"
    )
