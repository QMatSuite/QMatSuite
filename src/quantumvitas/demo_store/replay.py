"""
Replay engine: replays a list of AuthoringOps against QVService to build a project.

All ops route through existing QVService methods — no direct YAML writes.
"""
...
def replay_ops(ops: list[AuthoringOp], target_dir: Path) -> Path:
    ...
    for op in ops:
        if isinstance(op, InitProject):
            project_root = QVService.init_project(target_dir, name=op.name)
