...
def _load_ui_parameters() -> Dict[str, Any]:
    ...
    try:
        # JSON files are still in quantumvitas.data package (not moved)
        data_path = resources.files("quantumvitas.data").joinpath("qe_ui_parameters.json")
...
def _load_raw_metadata() -> Dict[str, Any]:
    ...
    # Get the file path
    # JSON files are still in quantumvitas.data package (not moved)
    data_path = resources.files("quantumvitas.data").joinpath("qe_module_parameters.json")
