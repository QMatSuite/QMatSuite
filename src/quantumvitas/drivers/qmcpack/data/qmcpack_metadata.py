...
def _load_raw_metadata() -> Dict[str, Any]:
    ...
    If ``QV_QMCPACK_METADATA_HOT_RELOAD=1`` is set, checks file mtime
    ...
    data_path = resources.files("quantumvitas.drivers.qmcpack.data").joinpath(
        "qmcpack_tags.json"
    )

    hot_reload = (
        os.environ.get("QV_QMCPACK_METADATA_HOT_RELOAD", "").strip() == "1"
    )
