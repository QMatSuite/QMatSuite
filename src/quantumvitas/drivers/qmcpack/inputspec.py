...
def _parse_ion_particleset(
    ...
    if pos_attrib is not None:
        ...
        if ionid_attrib is not None:
            ...
            species = []
            for g in groups:
                gsize = int(pset.get("size", "0")) if len(groups) == 1 else 0
                # Try to infer from total positions / number of groups
                pass
...
def _parse_qmcpack_text(text: str) -> dict[str, Any]:
...
def _write_qmcpack_text(fragment: dict[str, Any]) -> str:
    ...
    if species and coords:
        ...
        for sp in unique_species:
            group = ET.SubElement(ions, "group", name=sp)
