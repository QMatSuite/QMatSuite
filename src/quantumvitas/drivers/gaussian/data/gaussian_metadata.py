...
def _load_raw_metadata() -> Dict[str, Any]:
    ...
    If ``QV_GAUSSIAN_METADATA_HOT_RELOAD=1`` is set, checks file mtime
    ...
    data_path = resources.files("quantumvitas.drivers.gaussian.data").joinpath(
        "gaussian_route_keywords.json"
    )

    hot_reload = (
        os.environ.get("QV_GAUSSIAN_METADATA_HOT_RELOAD", "").strip() == "1"
    )
