...
def _quick_health_check(project_root: Path) -> list[str]:
    ...
    # 1. Can we parse project.qms.yml?
    try:
        from qmatsuite.core.project_utils import load_project_config

        config = load_project_config(project_root)
    except Exception as e:
        warnings.append(f"project.qms.yml is unreadable: {e}")
...
def init_project(name: str = "") -> dict:
    ...
    from qmatsuite.mcp.project import set_project_root
    ...
    try:
        project_root = QMSService.init_project(project_dir, name=name or None)
