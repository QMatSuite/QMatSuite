"""MACE metadata access layer.

Provides cached access to ``mace_tags.json`` using stdlib-only imports.
"""
...
MACE_METADATA_LOAD_STATE: dict[str, Any] = {
...
def _update_load_state(
    ...
    MACE_METADATA_LOAD_STATE["loaded_via"] = loaded_via
    MACE_METADATA_LOAD_STATE["loaded_at"] = datetime.now(timezone.utc).isoformat()
    if path_abs is not None:
        MACE_METADATA_LOAD_STATE["path_abs"] = path_abs
    if schema_version is not None:
        MACE_METADATA_LOAD_STATE["schema_version"] = schema_version


def _load_raw_metadata() -> dict[str, Any]:
    global _METADATA_CACHE, _METADATA_MTIME

    data_path = resources.files("qmatsuite.drivers.mace.data").joinpath(
        "mace_tags.json"
    )
    hot_reload = os.environ.get("QMS_MACE_METADATA_HOT_RELOAD", "").strip() == "1"
    ...
    try:
        ...
        raise FileNotFoundError(
            "mace_tags.json is missing from qmatsuite.drivers.mace.data."
        ) from exc
    except json.JSONDecodeError as exc:
        _METADATA_CACHE = None
        _METADATA_MTIME = None
        raise RuntimeError(f"mace_tags.json is invalid JSON: {exc}") from exc
    ...
    if schema_version not in (1,):
        ...
        _METADATA_MTIME = None
        raise RuntimeError(
            f"Unsupported schema version {schema_version} in mace_tags.json."
        )
...
def safe_load_metadata() -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"MACE metadata is not available: {exc}") from exc
...
def get_metadata_file_info() -> dict[str, Any]:
    try:
        data = safe_load_metadata()
        return {
            "metadata_path_abs": MACE_METADATA_LOAD_STATE.get("path_abs"),
