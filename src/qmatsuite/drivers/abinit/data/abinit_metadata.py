"""ABINIT input variable metadata access layer.

Provides the central API for accessing ABINIT parameter metadata.
All access to abinit_tags.json should go through this module.
...
ABINIT_METADATA_LOAD_STATE: Dict[str, Any] = {
...
def _update_load_state(
    ...
    ABINIT_METADATA_LOAD_STATE["loaded_via"] = loaded_via
    ABINIT_METADATA_LOAD_STATE["loaded_at"] = datetime.now(timezone.utc).isoformat()
    if path_abs is not None:
        ABINIT_METADATA_LOAD_STATE["path_abs"] = path_abs
    if schema_version is not None:
        ABINIT_METADATA_LOAD_STATE["schema_version"] = schema_version


def _load_raw_metadata() -> Dict[str, Any]:
    """Load the raw ABINIT tag metadata JSON file.

    Single entry point for JSON file access. Uses module-level caching.
    If ``QMS_ABINIT_METADATA_HOT_RELOAD=1`` is set, checks file mtime and
    ...
    data_path = resources.files("qmatsuite.drivers.abinit.data").joinpath(
        "abinit_tags.json"
    )

    hot_reload = os.environ.get("QMS_ABINIT_METADATA_HOT_RELOAD", "").strip() == "1"
    ...
    try:
        ...
        raise FileNotFoundError(
            "abinit_tags.json is missing from qmatsuite.drivers.abinit.data."
        ) from exc
    except json.JSONDecodeError as exc:
        _METADATA_CACHE = None
        _METADATA_MTIME = None
        raise RuntimeError(f"abinit_tags.json is invalid JSON: {exc}") from exc
    ...
    if schema_version not in (1,):
        ...
        _METADATA_MTIME = None
        raise RuntimeError(
            f"Unsupported schema version {schema_version} in abinit_tags.json. "
...
def safe_load_metadata() -> Dict[str, Any]:
    """Load ABINIT metadata for runtime use.
    ...
    try:
        ...
        raise RuntimeError(
            f"ABINIT tag metadata is not available: {exc}"
        ) from exc
...
def get_tag_info(tag_name: str) -> Optional[Dict[str, Any]]:
    """Look up a single ABINIT tag by name (case-insensitive).
    ...
    tags = data.get("tags", {})
    # ABINIT variables are lowercase by convention
    tag_lower = tag_name.lower()
    return tags.get(tag_lower)
...
def get_metadata_file_info() -> Dict[str, Any]:
    ...
    try:
        data = safe_load_metadata()
        return {
            "metadata_path_abs": ABINIT_METADATA_LOAD_STATE.get("path_abs"),
