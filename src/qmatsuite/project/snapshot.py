...
class ProjectSnapshot:
    """
    Snapshot of a complete QMatSuite project.
    ...
    def _from_minimal_format(cls, data: Dict[str, Any]) -> "ProjectSnapshot":
        ...
        - files: [{path: "project.qms.yml", content: "..."}, ...]
        ...
        # Parse project.qms.yml to get project metadata
        project_data = {}
        if "project.qms.yml" in file_map:
            try:
                project_data = yaml.safe_load(file_map["project.qms.yml"]) or {}  # EXC-004: in-memory string from snapshot archive
            except Exception:
                project_data = {}
...
def export_project_to_snapshot(project_root: Path) -> ProjectSnapshot:
    ...
    Reads project.qms.yml, all structures, calculations, and step specs
    ...
    for struct_entry in project_model.structures:
        ...
        if struct_path.exists():
            ...
            # Handle both __qms_meta__ wrapper and direct structure dict
            ...
            # Use struct_entry.meta (from project.qms.yml) for name, as it has the correct name
    ...
    for calculation_entry in project_model.calculations:
        ...
        # Prefer name from raw project.qms.yml entry (legacy format) as it may have the correct human-readable name
        ...
        # Check raw project.qms.yml entry for legacy name field
        ...
        if not calc_species_map and calculation_dict["steps"]:
            ...
            if any(step_species_overrides_list):
                from qmatsuite.core.models import migrate_species_overrides_to_calc
...
def materialize_project_from_snapshot(
    ...
    for struct_data in snapshot.structures:
        ...
        from qmatsuite.core.models import StructureEntry
    ...
    for calculation_data in snapshot.calculations:
        ...
        # Add to project model
        from qmatsuite.core.models import CalculationEntry
