"""
Test precision variants: bandspw must preserve K_POINTS kpath.

This test enforces the fix for the precision↔bandspw K_POINTS issue.
"""
...
class TestPrecisionVariantsBandsPw:
    """Test that precision variants correctly handle bandspw K_POINTS."""
    ...
    def test_bandspw_variant_excludes_kpoints(self):
        """Test that PRECISION_PW_BANDSPW variant does NOT include K_POINTS key."""
        variant = PRECISION_PW_BANDSPW_VARIANT

        # Check applies_to
        assert variant.applies_to("bandspw")
        ...
        assert not has_kpoints, "bandspw variant must NOT include K_POINTS key"
    ...
    def test_get_variant_bandspw(self):
        """Test that get_variant returns correct variant for bandspw."""
        variant = get_variant("precision", "bandspw")
        assert variant is not None
        assert variant.name == "PRECISION_PW_BANDSPW"
    ...
    def test_bandspw_kpoints_preserved(self, temp_calc_dir):
        """
        Apply precision to bandspw, K_POINTS must remain unchanged (k-path).

        This is the critical regression test.
        """
        # Create bandspw step with explicit k-path
