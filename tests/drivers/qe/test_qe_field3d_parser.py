"""Tests for QE field3d parser/provider."""
...
from quantumvitas.drivers.qe.parsers.field3d import QEField3DProvider

FIXTURE_DIR = Path(__file__).resolve().parents[2] / "data" / "analysis_qe_field3d"
...
def test_parse_grid_shape() -> None:
    provider = QEField3DProvider()
    result = provider.parse(_make_evidence(FIXTURE_DIR))
    assert result.grid_shape == (24, 24, 24)
    assert len(result.grid_data) == 13824
