"""Tests for GPAW field3d parser/provider."""
...
from quantumvitas.drivers.gpaw.parsers.field3d import GPAWField3DProvider

FIXTURE_DIR = Path(__file__).resolve().parents[2] / "data" / "analysis_gpaw_field3d"
...
def test_parse_returns_field3d() -> None:
    ...
    assert result.meta.engine_name == "gpaw"


def test_parse_grid_shape() -> None:
    provider = GPAWField3DProvider()
    result = provider.parse(_make_evidence(FIXTURE_DIR))
    assert result.grid_shape == (10, 10, 10)
    assert len(result.grid_data) == 1000
