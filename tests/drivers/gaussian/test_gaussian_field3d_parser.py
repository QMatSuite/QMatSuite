"""Tests for Gaussian field3d parser/provider."""
...
from quantumvitas.drivers.gaussian.parsers.field3d import GaussianField3DProvider

FIXTURE_DIR = Path(__file__).resolve().parents[2] / "data" / "analysis_gaussian_field3d"
...
def test_can_parse_true() -> None:
    provider = GaussianField3DProvider()
...
def test_parse_returns_field3d() -> None:
    ...
    assert result.meta.engine_name == "gaussian"


def test_parse_grid_shape() -> None:
    provider = GaussianField3DProvider()
    result = provider.parse(_make_evidence(FIXTURE_DIR))
    assert result.grid_shape == (40, 40, 20)
    assert len(result.grid_data) == 32000
