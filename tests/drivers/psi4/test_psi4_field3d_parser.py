"""Tests for Psi4 field3d parser/provider."""
...
from quantumvitas.drivers.psi4.parsers.field3d import Psi4Field3DProvider

FIXTURE_DIR = Path(__file__).resolve().parents[2] / "data" / "analysis_psi4_field3d"
...
def test_parse_returns_field3d() -> None:
    ...
    assert result.meta.engine_name == "psi4"


def test_parse_grid_shape() -> None:
    provider = Psi4Field3DProvider()
    result = provider.parse(_make_evidence(FIXTURE_DIR))
    assert result.grid_shape == (41, 56, 47)
    assert len(result.grid_data) == 107912
