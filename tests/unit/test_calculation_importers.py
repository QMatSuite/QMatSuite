...
def test_build_step_spec_from_qe_input_creates_structure_and_yaml(tmp_path: Path):
    ...
    assert result.step_type_spec == "qe_scf"


def test_build_step_spec_from_qe_input_detects_vc_relax(tmp_path: Path):
    """Test that vc-relax calculation type is correctly detected and converted to qe_relax."""
    ...
    assert result.spec_path.exists()
    # VC is a parameter, not a separate step type - should be qe_relax
    assert result.step_type_spec == "qe_relax", f"Expected 'qe_relax', got '{result.step_type_spec}'"
    
    # Verify step_type_spec is qe_relax in YAML
    spec_data = yaml.safe_load(result.spec_path.read_text())
    assert spec_data.get("step_type_spec") == "qe_relax"
    
    # Verify VC is preserved in parameters (calculation='vc-relax')
    params = spec_data.get("parameters", {})
    control = params.get("CONTROL", {}) or params.get("control", {})
    assert control.get("calculation") == "vc-relax", "VC should be preserved in parameters"


def test_build_step_spec_from_qe_input_detects_relax(tmp_path: Path):
    """Test that relax calculation type is correctly detected and converted to qe_relax."""
    ...
    assert result.spec_path.exists()
    # Should be qe_relax (spec type), not just "relax" (gen type)
    assert result.step_type_spec == "qe_relax", f"Expected 'qe_relax', got '{result.step_type_spec}'"
...
def test_materialize_step_spec_generates_atomic_species(tmp_path: Path):
    ...
    with correct pseudo filenames from species_overrides.
    
    This test ensures the fix for integration test failures:
    - ATOMIC_SPECIES card must be present in generated input
    - Species overrides from original QE input are preserved and applied
    - Pseudo filenames are correctly set in ATOMIC_SPECIES
    ...
    # Verify species_overrides were extracted from original input
    ...
    # This should generate ATOMIC_SPECIES with pseudo filenames from species_overrides
    ...
    # Verify Si entry has correct pseudo filename from species_overrides
    si_entry = None
    ...
    assert si_entry[2] == "Si.pz-vbc.UPF", f"Expected 'Si.pz-vbc.UPF' from species_overrides, got '{si_entry[2]}'"
    
    # Verify the generated input file exists and contains ATOMIC_SPECIES
    ...
    assert "Si.pz-vbc.UPF" in input_content, "Generated input must contain pseudo filename from species_overrides"
