"""
Unit tests for QMSService GUI-ready methods.
...
class TestGetProjectSummary:
    """Tests for QMSService.project.get_summary()."""

    def test_returns_project_info(self, tmp_path):
        """Test that project.get_summary returns correct structure."""
        # Create a minimal project
        project_root = QMSService.init_project(tmp_path / "test_project", name="Test Project")
    ...
    def test_counts_resources(self, tmp_path):
        ...
        project_root = QMSService.init_project(tmp_path / "test_project")

        # Create some calculations
        QMSService(project_root).project.init_calculation("calculation1")
        QMSService(project_root).project.init_calculation("calculation2")
...
class TestGetStructureVisData:
    """Tests for QMSService.get_structure_vis_data()."""
    ...
    def test_returns_visualization_data(self, tmp_path, sample_structure_file):
        ...
        project_root = QMSService.init_project(tmp_path / "test_project")
        QMSService(project_root).structure.import_file(sample_structure_file, name="si")
