...
def _make_mock_rester(mock_mpr):
    """Configure a MagicMock as a context-manager MPRester returning mock_mpr."""
...
class TestMaterialsProjectSearch:
    ...
    @patch(f'{_MP_MOD}.MP_API_AVAILABLE', True)
    def test_search_success(self):
        ...
        mock_mpr = _make_mock_rester(MagicMock())
        mock_mpr.summary.search.return_value = [mock_doc]

        with patch(f'{_MP_MOD}.MPRester', return_value=mock_mpr, create=True):
    ...
    def test_search_empty_api_key(self):
        ...
        with patch(f'{_MP_MOD}.MP_API_AVAILABLE', True):
    ...
    def test_search_mp_api_not_available(self):
        ...
        with patch(f'{_MP_MOD}.MP_API_AVAILABLE', False):
