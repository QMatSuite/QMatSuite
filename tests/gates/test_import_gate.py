...
Mission:
- CLI and Daemon MUST only import from qmatsuite.api and qmatsuite.api.utils
...
def test_cli_daemon_import_gate():
    """
    Gate test: Enforce that CLI and Daemon only import from qmatsuite.api.
    ...
    - Only allowed qmatsuite imports: qmatsuite.api.* and qmatsuite.api.utils
    ...
    # Always allowed qmatsuite import prefixes
    always_allowed_qms_prefixes = [
        "qmatsuite.api",
        "qmatsuite.api.utils",
    ]
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                for imported_module, lineno in imports_to_check:
                    # Only check qmatsuite.* imports
                    if imported_module.startswith("qmatsuite."):
                        # Check if it's always allowed
                        allowed = any(
                            imported_module.startswith(prefix)
                            for prefix in always_allowed_qms_prefixes
                        )

                        # Allow daemon to import from qmatsuite.daemon.*
                        if not allowed and is_daemon and imported_module.startswith("qmatsuite.daemon"):
                            allowed = True

                        # Allow CLI to import from qmatsuite.cli.*
                        if not allowed and is_cli and imported_module.startswith("qmatsuite.cli"):
                            allowed = True
    ...
    if violations:
        error_msg = "\n\n" + "="*80 + "\n"
        error_msg += "IMPORT GATE VIOLATION: CLI/Daemon must only import qmatsuite.api.*\n"
        error_msg += "="*80 + "\n\n"
        ...
        error_msg += "ALLOWED qmatsuite imports:\n"
        for prefix in always_allowed_qms_prefixes:
            error_msg += f"  ✓ {prefix}\n"
            error_msg += f"  ✓ {prefix}.*\n"
        error_msg += "  ✓ qmatsuite.daemon.* (only from daemon/ files)\n"
        error_msg += "  ✓ qmatsuite.cli.* (only from cli/ files)\n"
        error_msg += "\n"
        ...
        error_msg += "FIX: Use the domain API (qmatsuite.api.QMSService) instead of kernel imports.\n"
        error_msg += "="*80 + "\n"
