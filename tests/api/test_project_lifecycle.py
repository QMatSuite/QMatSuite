...
project.qv.yml structure at the kernel/utility level.
"""
...
class TestProjectLoadWalkUp:
    ...
    def test_stop_at_boundary_respected(self, tmp_path):
        ...
        outer.mkdir()
        QVService.init_project(outer / "proj")
...
class TestInitProjectIdempotency:
    ...
    def test_init_creates_marker(self, tmp_path):
        """init_project creates project.qv.yml marker file."""
        project_root = QVService.init_project(tmp_path / "new_proj")
    ...
    def test_project_yml_has_name(self, tmp_path):
        """project.qv.yml contains the project name (may be nested under 'project')."""
        QVService.init_project(tmp_path / "named_proj", name="My Test Project")
        config = yaml.safe_load((tmp_path / "named_proj" / "project.qv.yml").read_text())
    ...
    def test_project_yml_has_structures_list(self, tmp_path):
        """project.qv.yml has a structures list (possibly empty)."""
        QVService.init_project(tmp_path / "proj")
        config = yaml.safe_load((tmp_path / "proj" / "project.qv.yml").read_text())
    ...
    def test_project_yml_has_calculations_list(self, tmp_path):
        """project.qv.yml has a calculations list (possibly empty)."""
        QVService.init_project(tmp_path / "proj")
        config = yaml.safe_load((tmp_path / "proj" / "project.qv.yml").read_text())
    ...
    def test_service_usable_after_init(self, tmp_path):
        """QVService is usable immediately after init_project."""
        project_root = QVService.init_project(tmp_path / "proj")
