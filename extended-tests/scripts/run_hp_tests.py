#!/usr/bin/env python3
"""
Run hp tests in the same style as the official QE test suite.

HP (Hubbard U) tests require:
- arg=1,2: pw.x (SCF calculation)
- arg=3,4: hp.x (Hubbard parameter calculation)
"""
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run hp tests in official test suite style")
    parser.add_argument(
        "--qe-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Test category to run. If not specified, runs all hp categories"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    if not engine.detect_executable("hp.x"):
        print(f"ERROR: hp.x not found at {args.qe_path}")
        sys.exit(1)
    
    print(f"QE Engine: {engine.get_executable_path('pw.x')}")
    print(f"HP Engine: {engine.get_executable_path('hp.x')}")
    print(f"Test suite: {args.test_dir}")
    
    # Parse jobconfig for hp tests
    jobconfig_path = args.test_dir / "jobconfig"
    all_tests = parse_jobconfig(jobconfig_path, "hp_")
    ...
    print(f"Running {len(categories_to_run)} hp test categories...")
    print("=" * 60)
    
    # Executable map based on run-hp.sh:
    # arg=1,2: pw.x
    # arg=3,4: hp.x
    executable_map = {
        "1": "pw.x",
