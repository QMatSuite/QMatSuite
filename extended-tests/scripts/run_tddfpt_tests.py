#!/usr/bin/env python3
"""
Run tddfpt tests in the same style as the official QE test suite.

TDDFPT tests require:
- arg=1: pw.x (SCF calculation)
- arg=2: turbo_lanczos.x
- arg=3,4,7: turbo_spectrum.x
- arg=5: turbo_eels.x
- arg=6: turbo_magnon.x
"""
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run tddfpt tests in official test suite style")
    parser.add_argument(
        "--qe-home",
        type=Path,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Test category to run. If not specified, runs all tddfpt categories"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    # Parse jobconfig for tddfpt tests
    jobconfig_path = args.test_dir / "jobconfig"
    all_tests = parse_jobconfig(jobconfig_path, "tddfpt_")
    ...
    print(f"Running {len(categories_to_run)} tddfpt test categories...")
    print("=" * 60)
    
    # Executable map based on run-tddfpt.sh:
    # arg=1: pw.x
    # arg=2: turbo_lanczos.x
    # arg=3,4,7: turbo_spectrum.x
    # arg=5: turbo_eels.x
