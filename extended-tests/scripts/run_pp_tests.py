#!/usr/bin/env python3
"""
Run pp tests in the same style as the official QE test suite.

This script:
1. Reads jobconfig to get pp test order and files
2. Runs each test file in the specified order (pw.x first, then pp.x)
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run pp tests in official test suite style")
    parser.add_argument(
        "--qe-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Test category to run (e.g., pp_base). If not specified, runs all pp categories"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    if not engine.detect_executable("pp.x"):
        print(f"ERROR: pp.x not found at {args.qe_path}")
        sys.exit(1)
    
    print(f"QE Engine: {engine.get_executable_path('pw.x')}")
    print(f"PP Engine: {engine.get_executable_path('pp.x')}")
    print(f"Test suite: {args.test_dir}")
    
    # Parse jobconfig for pp tests
    jobconfig_path = args.test_dir / "jobconfig"
    if not jobconfig_path.exists():
        print(f"ERROR: jobconfig not found at {jobconfig_path}")
        sys.exit(1)
    
    all_tests = parse_jobconfig(jobconfig_path, "pp_")
    ...
    if args.category:
        ...
        # Run all pp categories
        categories_to_run = all_tests
    
    print(f"Running {len(categories_to_run)} pp test categories...")
    print("=" * 60)
    
    # Executable map: maps step args to executable names
    # Based on run-pp.sh:
    # arg=1: pw.x
    # arg=2: ppacf.x (or pp.x depending on test)
    executable_map = {
        "1": "pw.x",
        "2": "pp.x",  # Most pp tests use pp.x, some use ppacf.x
        "default": "pp.x"
    }
