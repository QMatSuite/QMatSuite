#!/usr/bin/env python3
"""
Run zg tests in the same style as the official QE test suite.
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run zg tests in official test suite style")
    parser.add_argument(
        "--qe-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Test category to run. If not specified, runs all zg categories"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    if not engine.detect_executable("ZG.x"):
        print(f"ERROR: ZG.x not found at {args.qe_path}")
        sys.exit(1)
    
    print(f"ZG Engine: {engine.get_executable_path('ZG.x')}")
    print(f"Test suite: {args.test_dir}")
    
    # Parse jobconfig for zg tests
    jobconfig_path = args.test_dir / "jobconfig"
    all_tests = parse_jobconfig(jobconfig_path, "zg_")
    ...
    print(f"Running {len(categories_to_run)} zg test categories...")
    print("=" * 60)
    
    # Executable map based on run-zg.sh:
    executable_map = {
        "0": "ZG.x",
        "default": "ZG.x"
    }
