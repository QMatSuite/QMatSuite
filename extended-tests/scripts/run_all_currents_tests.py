#!/usr/bin/env python3
"""
Run all_currents tests in the same style as the official QE test suite.

all_currents tests:
- Direct execution of all_currents.x
"""
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Run all_currents tests in official test suite style")
    parser.add_argument(
        "--qe-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Test category to run. If not specified, runs all all_currents categories"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    if not engine.detect_executable("all_currents.x"):
        print(f"ERROR: all_currents.x not found at {args.qe_path}")
        sys.exit(1)
    
    print(f"all_currents Engine: {engine.get_executable_path('all_currents.x')}")
    print(f"Test suite: {args.test_dir}")
    
    # Parse jobconfig for all_currents tests
    jobconfig_path = args.test_dir / "jobconfig"
    all_tests = parse_jobconfig(jobconfig_path, "all_currents_")
    ...
    print(f"Running {len(categories_to_run)} all_currents test categories...")
    print("=" * 60)
    
    # Executable map based on run-all_currents.sh:
    executable_map = {
        "default": "all_currents.x"
    }
