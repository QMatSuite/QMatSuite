...
test.describe('Pair 5 E2E: Al DOS Real-Run (from scratch)', () => {
  ...
  test('from-scratch Al DOS workflow with non-empty DOS plot', async ({ appPage }, testInfo) => {
    ...
    await appPage.getByTestId('qms-input-project-name').fill('al-dos-e2e');
    ...
    await appPage.getByTestId('qms-import-structure-file').fill(alCif);
    await appPage.getByTestId('qms-import-structure-name').fill('Al');
    ...
    await appPage.getByTestId('qms-create-calc-name').fill('Al DOS Test');
    ...
    const pseudoSelectAl = appPage.getByTestId('qms-pseudo-select-Al');
    ...
    const ensureDosParam = async (paramName: string) => {
      ...
      let input = panel.getByTestId(`qms-param-input-dos-${lower}`);
      ...
      input = panel.getByTestId(`qms-param-input-dos-${lower}`);
    ...
    const setSystemCharacterRawParam = async (paramName: string, value: string) => {
      ...
      const rawInput = row.getByTestId(`qms-param-input-system-${paramName.toLowerCase()}`).first();
    ...
    const waitForDosState = async (timeoutMs: number): Promise<'chart' | 'tile' | 'none' | 'timeout'> => {
      ...
      while (Date.now() - start < timeoutMs) {
        ...
        const chartVisible = await analysisPanel.getByTestId('qms-analysis-dos-chart').isVisible().catch(() => false);
        if (chartVisible) {
          return 'chart';
        }
    ...
    const fermiMarker = appPage.getByTestId('qms-analysis-fermi');
