...
test.describe('E2E: Demo Gallery', () => {
  test('demo gallery opens and allows creating DOS demo project', async ({ appPage }, testInfo) => {
    ...
    await expect(appPage.getByTestId('qms-welcome')).toBeVisible();
    ...
    await expect(appPage.getByTestId('qms-demo-gallery-view')).toBeVisible({ timeout: 10000 });
    
    // Wait for loading to complete
    const loadingState = appPage.getByTestId('qms-demo-gallery-loading');
    await expect(loadingState).not.toBeVisible({ timeout: 20000 });
    
    // Verify demo cards are visible (not error state)
    const cardsContainer = appPage.getByTestId('qms-demo-gallery-cards');
    const errorState = appPage.getByTestId('qms-demo-gallery-error');
    const emptyState = appPage.getByTestId('qms-demo-gallery-empty-state');
    ...
    const siBandsCard = appPage.getByTestId('qms-demo-card-si-bands-demo');
    const siDosCard = appPage.getByTestId('qms-demo-card-si-dos-demo');
    ...
    await expect(siBandsCard.getByTestId('qms-demo-tag-si-bands-demo-bands')).toBeVisible();
    ...
    const createDosBtn = appPage.getByTestId('qms-demo-card-btn-create-si-dos-demo');
    ...
    await expect(appPage.getByTestId('qms-project-path')).toBeVisible();
    ...
    const workflowsList = appPage.getByTestId('qms-calculations-list');
