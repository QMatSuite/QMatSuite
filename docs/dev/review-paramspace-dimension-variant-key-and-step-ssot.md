...
**证据位置**: `src/quantumvitas/presets/dimensions.py`
...
**证据**: 
- `src/quantumvitas/presets/variants_registry.py:42-111` - 每个 dimension 可以有多个 variants，每个 variant 有一个 ParamSpace
- `src/quantumvitas/presets/paramspace.py:525` - ParamSpace 有 `name` 字段，通常等于 dimension name
...
**证据位置**: `src/quantumvitas/presets/space_variant.py:16-50`
...
**证据**: 
- `src/quantumvitas/presets/variants_registry.py:67-90` - Precision 有 3 个 variants（PRECISION_PW_DEFAULT, PRECISION_PW_NSCF, PRECISION_PW_BANDS_PW），每个 variant 有自己的 ParamSpace
- `src/quantumvitas/presets/paramspace.py:709-713` - Precision ParamSpace 有 3 个 profiles（"LOW", "MED", "HIGH"）

**互斥表示**:
- Variant 互斥通过 `(step_type, dimension)` 索引表保证（`src/quantumvitas/presets/variants_registry.py:138-147`）
- Profile 互斥通过 `match_profile()` 检测多个匹配并抛出异常（`src/quantumvitas/presets/paramspace.py:300-304`）

**Variant 与 Step 绑定**:
- ✅ **是**，variant 通过 `applies_to_step_types` 字段绑定到 step types
- 绑定关系存储在 `VARIANT_BY_STEP_AND_DIMENSION` 索引表（`src/quantumvitas/presets/variants_registry.py:158`）
...
**证据**: `src/quantumvitas/presets/paramspace.py:82-117` - `ParamKey.canonicalize()` 和 `ParamKey.matches()` 方法
...
- `DIMENSION_OWNED_KEYS`（`src/quantumvitas/presets/integration.py:307-324`）是**硬编码映射**，不是运行时校验
...
**实际约束**:
- 删除逻辑使用 `DIMENSION_OWNED_KEYS` 决定哪些 key 可以被删除（`src/quantumvitas/presets/integration.py:511-519`）
- 但这只是**约定**，不是 enforce
...
**实际例子**:
- `degauss` 只出现在 `occupations_scheme` dimension（`src/quantumvitas/presets/paramspace.py:495-502`）
- 但这是**约定**，不是 enforce
...
**证据**: 
- `src/quantumvitas/presets/paramspace.py:268` - `for key in paramspace.keys`
- `src/quantumvitas/presets/variants_registry.py:496-500` - 匹配前先转换 QE→IR，但匹配时只检查 ParamSpace.keys
...
**证据位置**: `src/quantumvitas/presets/paramspace.py:463-529`
...
**生成函数**: `compile_profile_patch()`（`src/quantumvitas/presets/paramspace.py:316-377`）

#### Precision ParamSpace

**证据位置**: `src/quantumvitas/presets/paramspace.py:655-730`
...
**生成函数**: `_compile_precision_patch_for_step()`（`src/quantumvitas/presets/variants_registry.py:346-457`）
...
**证据**:
- `DIMENSION_ORDER`（`src/quantumvitas/presets/catalog.py:82-87`）只用于 UI 显示排序，不用于编译顺序
- `apply_presets_to_step()` 中直接遍历 `options.items()`，没有排序

**结论**: ⚠️ **当前不存在顺序控制**（这将与预期冲突）

**详细证据**:
- `apply_presets_to_step()` 中（`src/quantumvitas/presets/integration.py:417`）直接遍历 `applied_dimensions` 列表
- `applied_dimensions` 列表的构建（行 387-395）按 `options.items()` 的迭代顺序
- Python 3.7+ 字典保持插入顺序，但**没有显式排序**
- `DIMENSION_ORDER`（`src/quantumvitas/presets/catalog.py:82-87`）只用于 UI 显示，不用于编译顺序
...
- `precision_context`（`src/quantumvitas/presets/variants_registry.py:268`）提供外部数据（lattice_matrix, base_ecutwfc, base_ecutrho），但不是 "oracle"
...
- 这些数据来自 `precision_advice` 和 `precision_lattice_matrix` 参数（`src/quantumvitas/presets/integration.py:332-333`）
...
**证据**: 
- `src/quantumvitas/presets/paramspace.py:495-502` - `key_degauss` 定义在 `build_occupations_scheme_paramspace()` 中
- `src/quantumvitas/presets/integration.py:312` - `DIMENSION_OWNED_KEYS[DIMENSION_OCCUPATIONS_SCHEME]` 包含 `"degauss"`

#### degauss 在什么条件下写出？谁决定？

**证据位置**: `src/quantumvitas/presets/paramspace.py:316-377`
...
**证据**: `src/quantumvitas/presets/paramspace.py:507-523` - Profile 定义
...
**证据**:
- Precision ParamSpace 的 keys 列表（`src/quantumvitas/presets/paramspace.py:705`）不包含 `degauss`
- Precision 编译函数（`_compile_precision_patch_for_step()`）不涉及 `degauss`
- `DIMENSION_OWNED_KEYS[DIMENSION_PRECISION]`（`src/quantumvitas/presets/integration.py:314-319`）不包含 `degauss`
...
- **应用时**: `apply_presets_to_step()` 将 deletions 转换为 `None` 值（`src/quantumvitas/presets/integration.py:512-519`）
- **结果**: **degauss 被删除**（从 YAML 中移除）

**证据**: 
- `src/quantumvitas/presets/paramspace.py:355-358` - `NOT_APPLICABLE` → deletions
- `src/quantumvitas/presets/integration.py:512-519` - deletions → `None` → 删除
...
**证据**: 
- `src/quantumvitas/presets/paramspace.py:268` - `for key in paramspace.keys` - 只遍历 ParamSpace.keys
- Precision ParamSpace.keys 不包含 `degauss`（`src/quantumvitas/presets/paramspace.py:705`）

**结论**: ✅ **degauss 缺席不影响 precision profile 匹配**（因为 degauss 不在 precision ParamSpace.keys 中）

**证据**: `src/quantumvitas/presets/paramspace.py:705` - Precision ParamSpace.keys 只包含 `[key_ecutwfc, key_ecutrho, key_conv_thr, key_kpoints]`，不包含 `degauss`
...
**证据**: 
- `src/quantumvitas/presets/paramspace.py:278-284` - NOT_APPLICABLE 匹配逻辑
- `src/quantumvitas/presets/paramspace.py:355-358` - NOT_APPLICABLE 编译逻辑（删除）
...
**证据位置**:
- 编译: `src/quantumvitas/presets/variants_registry.py:260-343`
- 匹配: `src/quantumvitas/presets/paramspace.py:241-309`
- 存储: `src/quantumvitas/workflow/step_factory.py:106-118`
...
| 来源 | 定义文件 | 类型 | 主要调用点 | 证据 |
|------|---------|------|-----------|------|
| `GeneralizedStep` 枚举 | `src/quantumvitas/workflow/generalized_steps.py:21-56` | Enum | `materialize_public_step_key()`, `dematerialize_to_generalized_step()` | 行 21-56 定义枚举值 |
| `StepTypeSpec.public_type` / `id` | `src/quantumvitas/workflow/registry.py:24-50` | 字符串字段 | `StepTypeRegistry.get()`, UI/workflow | 行 29-31 定义 public_type 字段 |
| `MATERIALIZATION_MAP` 的 key | `src/quantumvitas/workflow/generalized_steps.py:61-92` | Dict key (tuple) | `materialize_step()`, `dematerialize_step()` | 行 61-92 定义映射表 |
...
| 来源 | 定义文件 | 类型 | 主要调用点 | 证据 |
|------|---------|------|-----------|------|
| `StepTypeSpec.machine_type` | `src/quantumvitas/workflow/registry.py:24-50` | 字符串字段 | `step.yaml` 存储, `resolve_engine_for_step()` | 行 30, 45 定义 machine_type |
| `_STEP_TYPES` 字典的 key | `src/quantumvitas/workflow/registry.py:181-558` | Dict key (字符串) | `StepTypeRegistry` 初始化 | 行 185-558 定义所有 step types |
| `MATERIALIZATION_MAP` 的 value | `src/quantumvitas/workflow/generalized_steps.py:61-92` | Dict value (字符串) | `materialize_step()` | 行 63-86 定义映射值 |
...
|------|---------|------|-----------|------|---------|
| `StepType` 枚举 | `src/quantumvitas/calculation/types.py:10-30` | Enum | `Step.step_type`, `_coerce_step_type()` | 行 10-30 定义枚举 | **混用 gen 和 spec** |
| `ParamSpaceVariant.applies_to_step_types` | `src/quantumvitas/presets/space_variant.py:34` | FrozenSet[str] | `get_variant()`, `apply_presets_to_step()` | 行 34 定义字段 | **使用 public_type**（如 "scf", "nscf"），对应 GenStep 字符串，但格式与 `GeneralizedStep` 枚举不一致 |
...
**违规证据**:
1. **`StepType` 枚举**（`src/quantumvitas/calculation/types.py:10-30`）:
   - 包含混合值：`SCF = "scf"`（gen-like），`PYSCF_SCF = "pyscf_scf"`（spec-like），`BANDS_PW = "bands_pw"`（spec-like）
   - **混用 gen 和 spec**，违反 SSOT

2. **`applies_to_step_types`**（`src/quantumvitas/presets/variants_registry.py:48-100`）:
...
**证据位置**: `src/quantumvitas/calculation/types.py:10-30`
...
**证据位置**: `src/quantumvitas/workflow/step_factory.py:48-55`
...
**证据位置**: 
- Materialize: `src/quantumvitas/workflow/generalized_steps.py:96-122`
- Dematerialize: `src/quantumvitas/workflow/generalized_steps.py:338-353`
- Runner: `src/quantumvitas/calculation/runner.py:96`

**结论**: 
- **Materialize**: 使用 `MATERIALIZATION_MAP`（gen → spec）
- **Dematerialize**: 使用 `MATERIALIZATION_MAP` 反向查找（spec → gen）
- **Runner**: 读取 `step.yaml` 的 `step_type` 字段（SPEC），然后通过 `_coerce_step_type()` 转换为 `StepType` 枚举（`src/quantumvitas/calculation/runner.py:50-77`）
...
- StepType 在 `Step.step_type` 字段中使用（`src/quantumvitas/calculation/step.py:28`），但执行时应该用 `step.yaml` 的 `machine_type`
...
1. **`StepType` 枚举**（`src/quantumvitas/calculation/types.py`）:
   - **当前**: 混用 gen 和 spec，不完整
   - **需要**: 移除或重构为纯 gen step 枚举（如果保留）
   - **影响**: `Step.step_type` 字段、`_coerce_step_type()` 函数

2. **`ParamSpaceVariant.applies_to_step_types`**（`src/quantumvitas/presets/space_variant.py`）:
...
3. **`_coerce_step_type()`**（`src/quantumvitas/calculation/runner.py:50-77`）:
   - **当前**: 将字符串转换为 `StepType` 枚举
   - **需要**: 改为直接使用字符串，或转换为 gen step
   - **影响**: Runner 执行逻辑

4. **`Step.step_type` 字段**（`src/quantumvitas/calculation/step.py:28`）:
...
1. **加载边界**（`src/quantumvitas/calculation/calculation.py:768-795`）:
   - `_coerce_step_type()` 函数将字符串转换为 `StepType` 枚举
   - **建议**: 保留兼容解析，但标记为 deprecated

2. **执行边界**（`src/quantumvitas/calculation/runner.py:50-77`）:
   - `_coerce_step_type()` 用于执行时类型转换
   - **建议**: 改为直接使用 `step.yaml` 的 `machine_type` 字符串，不转换

3. **API 边界**（`src/quantumvitas/api.py`）:
   - 多个地方使用 `StepType` 枚举
   - **建议**: 逐步迁移到字符串，保留兼容层
...
**补充证据**:
- `DIMENSION_OWNED_KEYS`（`src/quantumvitas/presets/integration.py:307-324`）标记为 DEPRECATED（行 303），只用于 UI 显示
...
**证据**: `src/quantumvitas/presets/integration.py:417` - 按 `options` 字典顺序遍历，没有显式顺序控制
...
**具体证据**:
- `src/quantumvitas/presets/variants_registry.py:48-100` - 所有 variant 的 `applies_to_step_types` 使用字符串（如 "scf", "nscf", "bands_pw"）
- 这些字符串对应 `StepTypeSpec.public_type`（`src/quantumvitas/workflow/registry.py:188, 201, 214`）
