...
| `qv init step [structure] --type TYPE [--workflow ID] [--project PATH] [overrides…]` | Generate a step `.yaml` in the enclosing workflow (auto-detected from pwd or via `--workflow`). Step name is auto-generated from type. Structure is optional if inside a workflow (inherits from workflow). | `qv init step --type nscf --workflow si_dos --project ~/projects/si_demo --SYSTEM.ecutwfc=60` |
