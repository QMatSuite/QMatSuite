...
| `qv init step <structure-id> [--workflow ID] [--name NAME] [--step-type scf] [overrides…]` | Generate a step `.yaml`, optionally appending it to a workflow (defaults to end). | `qv init step si --workflow si_dos --project ~/projects/si_demo --name nscf --step-type nscf --SYSTEM.ecutwfc=60` |
...
| `qv rename workflow <selector> [--name NAME] [--slug SLUG] [--path PATH]` | Update a workflow’s metadata and/or move its directory. Also exposes `--parent` metadata. | `qv rename workflow si_dos --project ~/projects/si_demo --name "Si DOS workflow"` |
...
| `qv delete structure <selector> [--project PATH] [--force] [--cascade]` | Move a structure (and optionally referencing workflows) into the project’s `trash/` folder. | `qv delete structure si --project ~/projects/si_demo` |
| `qv delete workflow <selector> [--project PATH] [--force] [--cascade]` | Move a workflow directory into `trash/`, optionally cascading dependent workflows. | `qv delete workflow si_dos --project ~/projects/si_demo --cascade` |
...
| `qv delete project [--project PATH] --force` | Move an entire project directory into the parent `trash/` folder. | `qv delete project --project ~/projects/si_demo --force` |
