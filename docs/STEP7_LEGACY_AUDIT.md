...
#### 1. K_POINTS Handling ✅
- **CLI** (`src/quantumvitas/cli/main.py:377`): Routes `--k_points` to `card_overrides` (correct)
...
- **Current Usage**:
  - `src/quantumvitas/project/snapshot.py`: Migration from step-level to calc-level (legitimate use)
  - `src/quantumvitas/core/pseudo.py`: Error messages reference step-level for diagnostics (acceptable)
  - `src/quantumvitas/api.py`: Temporary step-level pseudo setting during import (acceptable)
...
1. **Error Messages**: Some error messages in `src/quantumvitas/core/pseudo.py` reference step-level pseudo mapping for diagnostic purposes. These are acceptable but could be updated to only reference `species_map` for consistency.
