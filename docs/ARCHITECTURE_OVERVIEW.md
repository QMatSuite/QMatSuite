# QuantumVITAS v2 Architecture Overview

## 1) High-level Shape

**Major subsystems:**
- **GUI/Electron**: `gui/` - React/TypeScript Electron app
- **Daemon/Backend**: `src/quantumvitas/daemon/` - Python JSON-RPC stdio daemon
- **CLI/API**: `src/quantumvitas/cli/` + `src/quantumvitas/api.py` - CLI commands and service layer
- **Data/Assets**: `src/quantumvitas/data/` - QE metadata JSON, `resources/` - templates/demos
...
- **Preload**: `gui/electron/preload.ts` (exposes `window.qv` API via `contextBridge`)

**IPC model:**
- **Pattern**: Renderer → `window.qv.request(type, payload)` → Preload → Main (`ipcMain.handle('qv-request')`) → Python daemon (stdio JSON-RPC) → Response flows back
- **Types**: `gui/src/types/qv.ts` - centralized `QVCommandMap` for end-to-end type safety
- **Hook**: `gui/src/hooks/useQVClient.ts` - typed React hook wrapping `window.qv.request()`
...
**Startup:**
- **Entry**: `src/quantumvitas/daemon/server.py` - `python -m quantumvitas.daemon.server`
...
- **Loader**: `src/quantumvitas/data/qe_metadata.py` - `load_metadata()` with `@lru_cache`, `safe_load_metadata()`, `reload_metadata()`
...
2. Renderer (`gui/src/App.tsx`) mounts, `useQVClient()` hook calls `window.qv.request('list_jobs', {})`
3. Preload (`gui/electron/preload.ts`) forwards to main via `ipcRenderer.invoke('qv-request')`
4. Main (`gui/electron/main.ts`) writes JSON-RPC request to daemon stdin, waits for stdout response
5. Daemon (`src/quantumvitas/daemon/server.py`) `_handle_list_jobs()` → `JobManager.list_jobs()` → returns `JobSummary[]`
6. Response flows back: daemon stdout → main → IPC → renderer → `JobsPanel` displays list
7. User selects job → `JobsPanel` calls `window.qv.request('get_job_status', {job_id})` → daemon returns `Job` with `io_dir`, `steps`, `result`

**QE Parameter Browser: list modules/sections/parameters and reload metadata:**
1. `QEParameterBrowserPanel` mounts → calls `qv.listQeParameterMetadata('list_modules', {})`
2. Daemon `_handle_list_qe_parameter_metadata()` → `list_supported_modules()` from `src/quantumvitas/data/qe_metadata.py`
