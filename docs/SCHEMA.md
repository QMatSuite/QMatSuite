...
1. **ID-only cross-references**: All relationships between resources (project → calculation, calculation → structure, calculation → step) use only ULIDs, never selectors (name/slug/path).
...
3. **DAG structure**: Steps inherit structure from their parent calculation; steps do NOT store structure references.
...
project:
  ...
  settings: {}
calculations:
  - calculation_id: 01KBH0S...  # ✅ Only ID, no meta duplication
structures:
  - structure_id: 01KBH0RF...  # ✅ Only ID, no meta duplication
```

**Key points:**
- `calculation_id` and `structure_id` are ULIDs, not selectors
- No `name`, `slug`, or `path` fields in entries (these come from the resource files themselves)
- Full metadata is loaded from `calculation.yaml` and structure JSON files when needed

### 2. calculation.yaml

Calculation definition with ID-only references:
