...
- **Scope**: `workflow/registry.py` and `presets/capability.py`, `presets/integration.py` (capability queries only). No other workflow/presets module may import from `engine` for execution purposes. Note: `calculation/runner.py`, `execution/handlers.py`, and `drivers/*/handler.py` also import from engine — these are permitted by the dependency DAG (runtime→engine is allowed) and are NOT covered by this exception.
...
| `core.selectors` | `core.project_utils` | `selectors → project_utils → selectors` | Selectors need project config; project utils use selectors for resolution. |
...
- [x] Review each active exception against its expiration criteria
- [x] Confirm exception is still necessary (code still requires it)
- [x] Check that constraints are being followed (no scope creep):
  - [x] EXC-001: `workflow/registry.py` and `presets/capability.py`, `presets/integration.py` import from `engine.registry`; only `supported_presets` accessed. Scope widened to include presets (capability queries only).
  - [x] EXC-002: Lazy imports table updated — added `core.selectors → core.project_utils` (was undocumented). 3 known instances now documented.
  - [x] EXC-003: No new code paths import from `legacy/`; deletion deadline 2026-05-01 (88 days remaining). On track.
  - [x] EXC-004: All `yaml.safe_dump` usages outside `yaml_io.py` have `# EXC-004` or `# EXC-003` comment. `project/storage.py:33` and `legacy/migrate.py:231,244` properly tagged.
- [x] Verify gate test allowlists match this document (no undocumented entries)
- [x] Update review dates
- [x] Remove exceptions that meet their expiration criteria — none expired yet
- [x] File issues for exceptions approaching expiration — EXC-003 deadline 2026-05-01 noted

**Review completed**: 2026-02-03. All 4 exceptions remain active and compliant.
