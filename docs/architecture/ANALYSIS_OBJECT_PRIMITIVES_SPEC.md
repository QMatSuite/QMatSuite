...
4. [Transform Layer Contract](#4-transform-layer-contract)
5. [Visualization Contract](#5-visualization-contract)
6. [Provenance Persistence Policy](#6-provenance-persistence-policy)
7. [Layering Responsibilities](#7-layering-responsibilities)
8. [Acceptance Criteria and Future Gate Tests](#8-acceptance-criteria-and-future-gate-tests)
...
| Term | Definition |
|------|-----------|
| **Raw artifact** | Engine-native output file residing in `calc/raw/` (e.g., `vasprun.xml`, `scf.out`, `DOSCAR`). Raw artifacts are *evidence*; they are NOT SSOT. |
...
| **AnalysisObject** | Engine-agnostic, in-memory domain data structure derived from raw artifacts by an engine parser. Carries full numeric payload (arrays, frames, spectra) plus a single `AnalysisObjectMeta` container. Computed on demand, never persisted to disk as a primary store. |
...
| **to_primitives()** | The required, parameter-free method on every AnalysisObject that produces a `CanonicalPrimitiveBundle`. Deterministic and pure. |
| **CanonicalPrimitiveBundle** | The deterministic reshaping of an AnalysisObject into renderer-consumable arrays and annotations. May ONLY be produced by `AnalysisObject.to_primitives()`. |
| **DerivedPrimitiveBundle** | A primitive bundle produced by applying a `PrimitiveTransform` to a canonical or derived bundle. May ONLY be produced by `PrimitiveTransform`. |
...
| **Engine parser** | Code in `drivers/<engine>/parsers/` that reads raw artifacts and returns an AnalysisObject. Parsers are the only code permitted to read raw artifacts for analysis purposes. |
...
> Raw artifacts are the source evidence for analysis derivation. They are NOT SSOT. SSOT in this project refers exclusively to `calculation.yaml`, `step.yaml`, and structure resources.

- Engine parsers MUST read raw artifacts to produce AnalysisObjects.
...
| `created_at` | str (ISO 8601) | Timestamp of object construction |
...
### Inv-A4: Canonical Bundles May Only Come From to_primitives()

> `CanonicalPrimitiveBundle` may ONLY be produced by `AnalysisObject.to_primitives()`.
...
- `to_primitives()` MUST be deterministic and pure: identical raw evidence + identical parser version => identical canonical bundle (not byte-identical, but semantically identical after float normalization).
- Canonical operations allowed inside `to_primitives()`: reshape, reindex, unit normalization, reference annotation (e.g., `reference_energy = E_F`). Arrays MUST NOT be shifted, smoothed, or cropped.

### Inv-A5: Derived Bundles May Only Come From PrimitiveTransform

> `DerivedPrimitiveBundle` may ONLY be produced by `PrimitiveTransform`.
...
### Inv-A6: Bundles Contain Data and Minimal Annotations Only

> PrimitiveBundles (canonical or derived) carry numeric arrays and minimal generic annotations needed for rendering. Nothing more.
...
- Engine-specific fields or identifiers
- Raw file paths or contents
...
### Inv-A8: Visualization Consumes Only PrimitiveBundles

> All visualization code (Electron UI, Jupyter/matplotlib renderers, export functions) MUST consume PrimitiveBundles exclusively.
...
- Visualization MUST NOT contain engine-specific logic or branching.
...
### Inv-A10: No Persistent Analysis Cache

> Analysis results have no disk cache. In-memory memoization is the only caching mechanism.

- There MUST NOT be a persistent (on-disk) cache for AnalysisObjects or PrimitiveBundles.
- In-memory memoization is allowed as a runtime enhancement in long-lived processes (daemon, Jupyter kernel). CLI processes typically do not cache.
- No cross-process cache pointers or shared-memory analysis stores.
- Every process that needs analysis MUST be capable of re-deriving it from raw artifacts.
...
- The operational UI path MUST derive analysis from raw artifacts on demand. It MUST NOT implicitly read CanonicalPrimitiveBundles from CAS as a substitute for live derivation.
...
- User "pin" actions may persist additional screenshots or view snapshots as separate CAS entries (not auto-managed).
...
- **API/QVService** owns: request routing, optional in-memory memoization cache, ALL writes to SQLite and CAS (including overwrite/dedup policies for Tier-0.8 snapshots).
...
├── meta: AnalysisObjectMeta          Copied from source AnalysisObject
│
...
│
├── annotations: dict[str, Any]       Minimal generic annotations
...
- `reference_energy` is an annotation that renderers or transforms may use, but the data itself is unmodified.
- `bundle_kind` is always the literal string `"canonical"`.

### 3.4 DerivedPrimitiveBundle

Produced exclusively by `PrimitiveTransform`. Tagged with `bundle_kind = "derived"`.

```
DerivedPrimitiveBundle
├── bundle_kind: "derived"            Literal tag (always "derived")
├── object_type: str                  Inherited from input bundle
├── meta: AnalysisObjectMeta          Inherited from input bundle
│
...
- Derived bundles MUST NEVER be persisted to CAS.
...
## 4. Transform Layer Contract

### 4.1 PrimitiveTransform Interface
...
### 4.2 Transform Composition

Transforms compose by chaining: the output of one transform feeds the input of the next.

```
canonical = analysis_object.to_primitives()         # CanonicalPrimitiveBundle
shifted  = FermiShift().apply(canonical)             # DerivedPrimitiveBundle
smoothed = GaussianSmooth(sigma=0.05).apply(shifted) # DerivedPrimitiveBundle
cropped  = EnergyCrop(emin=-5, emax=5).apply(smoothed) # DerivedPrimitiveBundle
```

The `transform_chain` on the final bundle records all three transforms in order.

### 4.3 Standard Transform Catalog (Non-Exhaustive)

| Transform | Input requirement | Parameters | Effect |
|-----------|-------------------|------------|--------|
| `FermiShift` | Bundle with `reference_energy` annotation | (none) | Subtracts reference_energy from energy arrays |
...
### 5.2 Visualization Prohibitions
...
| Contain engine-specific branching (`if engine == "vasp"`) | Bundles are engine-agnostic |
...
## 6. Provenance Persistence Policy

### 6.1 Two Categories of Analysis Output
...
| **(B) AnalysisObject → Primitives** | Bands, DOS, trajectory, phonon dispersion | Lazy: on-demand when UI requests | In-memory only; CAS Tier-0.8 snapshot optional |

### 6.2 Post-Run Digest Persistence (Category A)
...
### 6.3 CAS Tier-0.8 Snapshots (Category B)

When the API layer computes an AnalysisObject and calls `to_primitives()`, it MAY auto-persist the canonical bundle to CAS under these rules:
...
- **User pins:** Additional screenshots or view exports may be persisted as separate CAS entries via explicit user "pin" action. These are not auto-managed; no overwrite.

### 6.4 What Is NOT Persisted

- AnalysisObjects (in-memory only; re-derived from raw on every request).
- DerivedPrimitiveBundles (ephemeral; re-computed by re-applying transforms).
- Transform chain state (lives only in the DerivedPrimitiveBundle in memory).
- In-memory memoization cache contents (lost on process exit; by design).

---

## 7. Layering Responsibilities
...
| Apply requested transforms | Calls PrimitiveTransform chain; produces DerivedPrimitiveBundle |
| In-memory memoization (optional) | Cache AnalysisObjects and/or canonical bundles keyed by `(step_ulid, object_type, source_file_stats)` |
...
| Enforce DerivedPrimitiveBundle non-persistence | MUST NOT write derived bundles to CAS |
...
| Render bundle using presentation logic | Call parsers |
...
## 8. Acceptance Criteria and Future Gate Tests

### 8.1 Gate Test Inventory
...
### 8.2 Acceptance Criteria (Behavioral)

1. **Roundtrip determinism**: For any engine's test corpus, `parser.parse(raw_dir).to_primitives()` called twice yields semantically identical canonical bundles (array values equal within float tolerance).
...
7. **CAS deletion safety**: Deleting `.provenance/.cas/` does NOT affect the operational analysis path. Analysis is re-derived from raw artifacts. Only provenance replay features become unavailable.
