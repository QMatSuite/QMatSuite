...
### 1. Unified PySCF Execution Path (`src/quantumvitas/engine/pyscf_engine.py`, `src/quantumvitas/engines/pyscf/__main__.py`, `src/quantumvitas/engines/pyscf/runner.py`)

**Problem:**
- PySCF had two execution paths: `run_step` (single step) and `run_step_with_chain` (chain).
- The `_get_runner_command()` in `pyscf_engine.py` was incorrectly pointing to `quantumvitas.engines.pyscf.runner`, which only handled single steps via `run_job()`.
...
**Changes:**
- `_get_runner_command()` in `pyscf_engine.py` now returns `[sys.executable, "-m", "quantumvitas.engines.pyscf"]`. This directs all PySCF subprocess calls to `__main__.py`.
- `src/quantumvitas/engines/pyscf/__main__.py` was modified to *always* call `run_job_chain()`, effectively making single-step execution a chain of length 1. The `run_job()` path was removed.
...
- The `run_job()` function was removed from `src/quantumvitas/engines/pyscf/runner.py`.
...
### 2. Structure Input Handling in `build_mole()` (`src/quantumvitas/engines/pyscf/runner.py`)
...
**Changes:**
- `build_mole()` was updated to prioritize `structure_path` from `params`. If present, it uses `quantumvitas.io.structure_io.read_structure` to load the molecule and convert it to the internal `atoms` format.
...
### 3. Structure Contract Enforcement at Chain Assembly (`src/quantumvitas/engine/pyscf_engine.py`)
...
### 4. Registry Contract Enforcement (`src/quantumvitas/workflow/registry.py`)
...
### 5. Input Script Generation (`src/quantumvitas/engines/pyscf/chain_execution.py`)
...
### 6. Legacy `_run_scf` Method Fix (`src/quantumvitas/engine/pyscf_engine.py`)
...
1. `src/quantumvitas/engine/pyscf_engine.py` - Unified execution path, structure contract enforcement
2. `src/quantumvitas/engines/pyscf/__main__.py` - Always use chain execution
3. `src/quantumvitas/engines/pyscf/runner.py` - Removed `run_job()`, improved `build_mole()` structure handling
4. `src/quantumvitas/engines/pyscf/chain_execution.py` - Added input script generation
5. `src/quantumvitas/workflow/registry.py` - Updated `pyscf_mp2` and `pyscf_td` StepTypeSpec definitions

## Confirmation

There is exactly one PySCF subprocess entrypoint: `python -m quantumvitas.engines.pyscf` (via `__main__.py`), and it always executes `run_job_chain()` logic. `PySCFEngine.run_step()` and `run_step_with_chain()` always build a chain and always call the chain runner. Single-step SCF becomes chain length 1.
