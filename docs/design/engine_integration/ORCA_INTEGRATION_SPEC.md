...
**Implementation**: `qmatsuite.workflow.registry.generate_subchain_basename()`
...
2. Add the mapping to `PUBLIC_TYPE_TOKENS` in `src/qmatsuite/workflow/registry.py`
...
**Location**: `src/qmatsuite/engines/orca/input_compiler.py`
...
```
<HOME>/QMatSuite/.qmatsuite/engines/orca/orca_6_1_1_macosx_arm64_openmpi411/orca
```
...
**Note**: Integration tests use the resolver (`qmatsuite.core.engines.orca_resolver.resolve_orca_bin()`) which automatically detects bundled ORCA. Setting `QMATSUITE_ORCA_BIN` is optional but recommended for explicit control.
...
Expected output:
```
ORCA found at: <HOME>/QMatSuite/.qmatsuite/engines/orca/orca_6_1_1_macosx_arm64_openmpi411
```
...
- `src/qmatsuite/engine/orca_engine.py` - ORCA engine implementation
- `src/qmatsuite/engines/orca/input_compiler.py` - Input file generation
- `src/qmatsuite/engines/orca/property_parser.py` - Property file parsing
- `src/qmatsuite/core/engines/orca_resolver.py` - ORCA binary resolution

### Chain Infrastructure

- `src/qmatsuite/engine/qc_engine_base.py` - QCChain dataclass, chain detection
- `src/qmatsuite/workflow/registry.py` - Step type registry, token mapping
