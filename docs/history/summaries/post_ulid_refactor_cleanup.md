...
**Files Modified**:
- `src/qmatsuite/daemon/server.py`: `_handle_delete_calculation`, `_handle_can_delete_calculation`
- `src/qmatsuite/api.py`: `can_delete_calculation`, `delete_calculation`
- `src/qmatsuite/core/project_utils.py`: `entry_matches`
...
**Files Modified**:
- `src/qmatsuite/daemon/server.py`: `_handle_detect_workflow_for_calculation`
- `gui/src/components/panels/CalculationListPanel.tsx`: Use `workflow_label` from response
- `gui/src/types/qms.ts`: Added `workflow_label` and `present_steps` to `DetectWorkflowForCalculationResult`
...
**Files Modified**:
- `src/qmatsuite/daemon/server.py`: `_handle_create_calculation` (added `calculation_ulid` to response)
...
**Problem**: `QMSService.get_common_cards` called `get_step_detail` with `calculation_selector=` keyword, but `get_step_detail` signature had been updated to use `calculation_ulid=`, causing `TypeError`.
...
- `src/qmatsuite/daemon/server.py`: All corresponding handlers updated to resolve selectors to ULIDs

**Tests**: `tests/daemon/test_get_common_cards.py`

## Design Principles Enforced

1. **ULID-only core**: Core service methods (`QMSService.*`) require ULIDs only, not selectors
