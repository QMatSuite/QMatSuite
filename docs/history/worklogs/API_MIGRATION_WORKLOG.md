...
**Problem:** 6 test files import from `qmatsuite.api_legacy` and call `QMSService._build_structure_vis_payload()`.
...
**Solution:** Canonical function already exists at `qmatsuite.analysis.structure_viz.build_structure_vis_payload`.
...
- **Before:** `from qmatsuite.api_legacy import QMSService`
- **After:** `from qmatsuite.analysis.structure_viz import build_structure_vis_payload, DisplayModeParams`
- **Changes:** 3 callsites: `QMSService._build_structure_vis_payload(...)` → `build_structure_vis_payload(...)`
- **Status:** ✅ DONE
...
- **Before:** Two occurrences of `from qmatsuite.api_legacy import QMSService`
- **After:** `from qmatsuite.analysis.structure_viz import build_structure_vis_payload, DisplayModeParams`
...
**Legacy import check:**
```bash
grep -rn "from qmatsuite\.api_legacy\|from qmatsuite\._api_legacy" tests/ src/
...
1. **Legacy files moved to vault:**
   - `src/qmatsuite/api_legacy.py` → `src/qmatsuite/_vault/_legacy_facade.py`
   - `src/qmatsuite/_api_legacy.py` → `src/qmatsuite/_vault/_legacy_service.py`
...
4. **All gates passing:**
   - Single QMSService definition: ✅
   - No legacy imports: ✅
   - Dangling call scanner: 212 dangling calls remain (methods called on QMSService that don't exist)
...
- **Canonical QMSService methods:** 27
...
| `get_pseudo_config` | 15 | B (capability) | Add to QMSService.pseudo or domain fn |
...
| `can_delete_structure` | 4 | A (exists) | Already on QMSService.structure |
| `download_pseudo_by_filename` | 4 | B (capability) | Add to QMSService.pseudo |
| `run_input_step` | 3 | B (capability) | Consolidate with run_step |
| `list_installed_sssp` | 3 | B (capability) | Add to QMSService.pseudo |
| `check_archives_status` | 3 | B (capability) | Add to QMSService.pseudo |
| `detect_engine_for_calculation` | 3 | B (capability) | Add to QMSService.engine |
...
- **Action:** Already in `qmatsuite.api.utils.is_ulid_like`. File needs to import from utils and call directly instead of `QMSService.is_ulid_like()`
- **Status:** ✅ DONE
...
- **Classification:** C (pure utils)
- **Action:** Already in `api.utils`, just needed to replace `QMSService.load_calculation()` calls
- **Status:** ✅ DONE
...
| Single QMSService definition | ✅ PASS |
...
- **Issue:** Import `from qmatsuite.frontends.daemon.jobs` failed (module didn't exist)
- **Verification:** `python -c "import qmatsuite.frontends.daemon.jobs"` → ModuleNotFoundError
- **Fix:** Changed to `from qmatsuite.daemon.jobs import JobManager, JobStatus`
- **Status:** ✅ FIXED
...
#### 3. Missing QMSServiceError compatibility alias
- **Issue:** frontends/daemon/server.py imported `QMSServiceError` which was moved to vault
- **Fix:** Added compatibility aliases in api/__init__.py:
  - `QMSServiceError = APIError`
  - `ErrorSpec = APIError`
...
- `qmatsuite.daemon.server`: 20+ imports across tests and daemon/__init__.py
- `qmatsuite.frontends.daemon.server`: ZERO external imports (orphaned)
