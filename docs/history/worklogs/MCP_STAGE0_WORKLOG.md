...
single `ping` tool, proving the full round-trip works. Pure plumbing — no QVService
integration yet.
...
- **Separate `app.py`** for the `FastMCP("QMatSuite")` singleton to avoid the `python -m`
  dual-import problem (server.py as `__main__` vs tools importing `quantumvitas.mcp.server`)
...
1. `src/quantumvitas/mcp/__init__.py` — package init
2. `src/quantumvitas/mcp/app.py` — `mcp = FastMCP("QMatSuite")` singleton
3. `src/quantumvitas/mcp/envelope.py` — `make_response()` envelope helper
4. `src/quantumvitas/mcp/server.py` — entry point, tool registration, `mcp.run()`
5. `src/quantumvitas/mcp/tools/__init__.py` — tools sub-package init
6. `src/quantumvitas/mcp/tools/ping.py` — `@mcp.tool ping()` health check
...
4. Second run: 5 pass, 1 fail — `tools/list` returns empty tools. Root cause: `python -m` dual-import problem — `server.py` as `__main__` creates one `FastMCP` instance, `ping.py` importing `quantumvitas.mcp.server` creates another.
5. Fix: extracted `mcp = FastMCP("QMatSuite")` into `app.py`; both `server.py` and `ping.py` import from `app.py`.
6. All 6 tests pass.

### Verification
- `python -c "from quantumvitas.mcp.server import mcp; print(mcp)"` → `FastMCP('QMatSuite')`
