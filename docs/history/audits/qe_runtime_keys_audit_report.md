...
**CLI show-command** (`src/quantumvitas/cli/main.py:3108-3197`):
...
- **Parameter extraction**: `_qe_input_to_parameter_dict(qe_input)` (line 3122)
  - **Location**: `src/quantumvitas/cli/main.py:4299-4308`
...
- **CLI args generation**: `_parameter_dict_to_cli_args(parameter_dict)` (line 3123)
  - **Location**: `src/quantumvitas/cli/main.py:4342-4350`
...
**CLI import paths**:
- **API import**: `QVService.import_step_from_qe_input()` (`src/quantumvitas/api.py:5538-5602`)
  - Uses `build_step_spec_from_qe_input()` with `apply_defaults=False` (line 5601)
  - **Location**: `src/quantumvitas/calculation/importers.py:95-196`
  - **Parameter extraction**: `_extract_parameters(qe_input)` (line 68)
    - **Location**: `src/quantumvitas/calculation/importers.py:444-453`
...
1. **CLI init step** (`src/quantumvitas/cli/main.py:920-1217`):
   ...
   - **Persistence**: `_write_step_spec(spec_path, spec, ...)` (line 1217)
     - **Location**: `src/quantumvitas/cli/main.py:4199-4209`
...
2. **API add_step_to_calculation** (`src/quantumvitas/api.py:6155-6350`):
...
3. **API import_step_from_qe_input** (`src/quantumvitas/api.py:5538-5602`):
...
4. **StructureStepSpec.to_dict()** (`src/quantumvitas/calculation/structure_steps.py:171-200`):
...
- **Snapshot export** (`src/quantumvitas/project/snapshot.py:375-381`):
...
1. **Prefix injection** (`src/quantumvitas/calculation/structure_steps.py:306-407`):
...
2. **Outdir injection** (`src/quantumvitas/calculation/structure_steps.py:394-407`):
...
3. **Pseudo_dir injection** (`src/quantumvitas/calculation/structure_steps.py:1111-1158`):
...
**API Entrypoints**:
- `QVService.add_step_to_calculation()` (`src/quantumvitas/api.py:6155-6350`)
  - Uses `get_default_step_params(step_type)` which includes `outdir` (see Risk 3)
- `QVService.import_step_from_qe_input()` (`src/quantumvitas/api.py:5538-5602`)
  - Extracts from .in file via `_extract_parameters()` which includes ALL params (see Risk 1)
- `QVService.update_step_params()` (`src/quantumvitas/api.py:4477-4555`)
...
**Daemon Entrypoints**:
- `_handle_add_step_to_calculation()` (`src/quantumvitas/daemon/server.py:4195-4226`)
  - Calls `QVService.add_step_to_calculation()` (uses defaults with outdir)
- `_handle_import_step_from_qe_input()` (`src/quantumvitas/daemon/server.py:4228+`)
  - Calls `QVService.import_step_from_qe_input()` (extracts all params)

**Config/Env Var Handling**:
- **Environment variables**: `QV_PSEUDO_PATH` exists (`src/quantumvitas/core/pseudo.py:344`) but is for pseudo resolution, NOT for pseudo_dir in QE input
...
**Location**: `src/quantumvitas/cli/main.py:1171-1211`
...
**Location**: `src/quantumvitas/api.py:6328`
- **Issue**: Default params include `outdir` in step_defaults.py
- **Evidence**: `src/quantumvitas/calculation/step_defaults.py:16,38,63,73,94,115,140,168`
...
**Location**: `src/quantumvitas/api.py:4477-4555`, `4930-4982`
...
**Location**: `src/quantumvitas/daemon/server.py:4195-4226`
- **Issue**: Calls `QVService.add_step_to_calculation()` which uses defaults
...
**Location**: `src/quantumvitas/calculation/step_defaults.py:16,38,63,73,94,115,140,168`
...
- **Location**: `src/quantumvitas/calculation/structure_steps.py:171-200`
...
- **Location**: `src/quantumvitas/workflow/step_factory.py:106-121`
