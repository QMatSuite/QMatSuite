...
1. **Static Method Violations**: 53 instances in daemon, 7 in CLI where `QMSService.<static_method>()` is called instead of `get_service(project_root).<subservice>.<method>()`
2. **No Forbidden Imports**: Both CLI and daemon correctly import only from `qmatsuite.api.*`
...
1. **Frontends → API only**: CLI and daemon must import ONLY from `qmatsuite.api.*` (plus stdlib/3rd-party)
2. **QMSService via subservices**: Operations should use instance-style: `get_service(project_root).<subservice>.<method>(...)`
3. **Utils only for DTO/errors/small helpers**: `api.utils.*` is allowed for pure utility functions
4. **No second service channel**: Frontends must not call legacy service paths (`_legacy_service`, static `QMSService.method()`, etc.)

### Current Divergences

**CRITICAL DIVERGENCE**: The codebase contains **60+ static method calls** that violate the instance-style pattern:

- **Daemon**: 53 instances of `QMSService.<static_method>()` calls
- **CLI**: 7 instances of `QMSService.<static_method>()` calls
...
**Verification**:
```bash
rg "^from qmatsuite\.(core|calculation|analysis|engine|project|history|execution|presets|workflow|ir)" src/qmatsuite/cli/
rg "^import qmatsuite\.(core|calculation|analysis|engine|project|history|execution|presets|workflow|ir)" src/qmatsuite/cli/
```

**Findings**: Zero matches. CLI correctly imports only from:
- `qmatsuite.api.*` (QMSService, errors, get_service)
- `qmatsuite.api.utils.*` (utility functions)
- `qmatsuite.api.qe_io.*` (QE I/O utilities)
...
**Verification**:
```bash
rg "^from qmatsuite\.(core|calculation|analysis|engine|project|history|execution|presets|workflow|ir)" src/qmatsuite/daemon/
rg "^import qmatsuite\.(core|calculation|analysis|engine|project|history|execution|presets|workflow|ir)" src/qmatsuite/daemon/
```

**Findings**: Zero matches. Daemon correctly imports only from:
- `qmatsuite.api.*` (QMSService, APIError, get_service)
- `qmatsuite.api.utils.*` (utility functions)
- `qmatsuite.daemon.jobs.*` (daemon-specific job management)
...
Both CLI and daemon **correctly** import only from `qmatsuite.api.*` (plus stdlib/3rd-party). This requirement is **FULLY COMPLIANT**.
...
|------|-------------|----------------|
| 237 | `QMSService.get_settings()` | Static call - should use instance method |
| 845 | `QMSService.init_pseudo_dirs()` | Static call - should use instance method |
| 889 | `QMSService.install_sssp_from_seed(...)` | Static call - should use instance method |
| 893 | `QMSService.install_all_sssp_from_seed(...)` | Static call - should use instance method |
| 974 | `QMSService.download_sssp_library(...)` | Static call - should use instance method |
| 1018 | `QMSService.download_all_sssp(...)` | Static call - should use instance method |
| 1062 | `QMSService.import_seed_archives(...)` | Static call - should use instance method |
| 1073 | `QMSService.list_pseudo_libraries()` | Static call - should use instance method |
| 1096 | `QMSService.get_library_status(...)` | Static call - should use instance method |
| 1135 | `QMSService.install_pseudo_library(...)` | Static call - should use instance method |
| 1164 | `QMSService.remove_pseudo_library(...)` | Static call - should use instance method |
| 1187 | `QMSService.repair_pseudo_library(...)` | Static call - should use instance method |
| 1198 | `QMSService.compute_store_size()` | Static call - should use instance method |
| 1331 | `QMSService.is_pseudo_archive_installed(...)` | Static call - should use instance method |
| 1354 | `QMSService.install_pseudo_archive(...)` | Static call - should use instance method |
| 1424 | `QMSService.get_settings()` | Static call - should use instance method |
| 1998 | `QMSService.get_project_summary(project_root)` | Static call - should use `get_service(project_root).project.get_summary()` |
| 2010 | `QMSService.list_structures_data(project_root)` | Static call - should use `get_service(project_root).structure.list()` |
| 2023 | `QMSService.list_calculations_data(project_root)` | Static call - should use `get_service(project_root).calculation.list()` |
| 2077 | `QMSService.init_project(...)` | Static call - acceptable (no project_root yet) |
| 2089 | `QMSService.get_project_summary(project_root)` | Static call - should use instance method |
| 2116 | `QMSService.import_structure(...)` | Static call - should use `get_service(project_root).structure.import_file()` |
| 2124 | `QMSService.list_structures_data(project_root)` | Static call - should use instance method |
| 2720 | Comment references `QMSService.delete_structure` | Documentation only |
| 2766 | `QMSService.init_calculation(...)` | Static call - should use `get_service(project_root).calculation.init()` |
| 2776 | `QMSService.list_calculations_data(project_root)` | Static call - should use instance method |
| 3025 | Comment references `QMSService.get_step_detail` | Documentation only |
| 3036 | Comment references `QMSService.get_step_detail` | Documentation only |
| 3046 | `QMSService.get_settings()` | Static call - should use instance method |
| 3209 | `QMSService.promote_relax_structure(...)` | Static call - should use instance method |
| 3600 | `QMSService.save_relax_final_structure(...)` | Static call - should use instance method |
| 3654 | Comment references `QMSService.delete_step_from_calculation` | Documentation only |
| 4215 | Comment references `QMSService.import_step_from_qe_input` | Documentation only |
| 4273 | Comment references `QMSService.is_path_like` | Documentation only |
| 4347 | `QMSService.get_settings()` | Static call - should use instance method |
| 4461 | `QMSService.analyze_project_pseudo_effects(...)` | Static call - should use instance method |
| 4486 | `QMSService.materialize_pseudo_file(...)` | Static call - should use instance method |
| 4524 | `inspect.signature(QMSService.get_calculation_detail)` | Static call - should use instance method |
| 4576 | `QMSService.get_pseudo_options_for_elements(...)` | Static call - should use instance method |
| 4634 | `QMSService.create_demo_project(...)` | Static call - acceptable (no project_root yet) |
| 4657 | `QMSService.list_demo_projects()` | Static call - acceptable (no project_root needed) |
| 5254 | Comment references `QMSService.run_calculation()` | Documentation only |
| 5320 | `func=QMSService.run_calculation` | Static call - should use instance method |
| 5330 | Comment references `QMSService.run_calculation` | Documentation only |
| 5391 | `func=QMSService.run_step` | Static call - should use instance method |
| 5400 | Comment references `QMSService.run_step` | Documentation only |
| 5446 | `func=QMSService.run_single_step` | Static call - should use instance method |
| 5454 | Comment references `QMSService.run_single_step` | Documentation only |
| 6300 | `service = QMSService.get_workflow_service()` | Static call - acceptable (returns service object) |
| 6334 | `service = QMSService.get_workflow_service()` | Static call - acceptable |
| 6417 | `service = QMSService.get_workflow_service()` | Static call - acceptable |
| 6488 | `service = QMSService.get_workflow_service()` | Static call - acceptable |
...
| Line | Method Call | Violation Type |
|------|-------------|----------------|
| 57 | Comment: `# CalculationRunner now accessed via QMSService.run_calculation()` | Documentation only |
| 1257 | `QMSService.generate_kpath(...)` | Static call - acceptable (pure utility, no project_root needed) |
| 1276 | `QMSService.get_default_step_params(step_type)` | Static call - acceptable (pure utility, no project_root needed) |
| 1753 | `QMSService.run_step(...)` | Static call - **VIOLATION** - should use `get_service(project_root).run.step(...)` |
| 3656 | `QMSService.configure_species_map(...)` | Static call - **VIOLATION** - should use instance method |
| 3939 | Comment: `# Resolve calculation selector (for use with QMSService.run_calculation static method)` | Documentation only |
| 4010 | `QMSService.run_calculation(...)` | Static call - **VIOLATION** - should use `get_service(project_root).run.calculation(...)` |
...
**Verification**:
```bash
rg "_legacy_service" src/qmatsuite/cli/ src/qmatsuite/daemon/
```
...
QMSService.run_calculation(project_root, ...)
QMSService.get_project_summary(project_root)
QMSService.list_structures_data(project_root)
...
- **Canonical methods**: 51 methods found on `qmatsuite.api.service.QMSService`
- **Dangling calls**: 0 calls to non-existent methods
- **Status**: All API method calls reference methods that exist on the canonical QMSService class

### Per-File Breakdown

No dangling calls found in any file. The scanner checked:
- `src/qmatsuite/**/*.py` (excluding `_vault/` and test files)
...
The canonical `QMSService` class (in `src/qmatsuite/api/service.py`) provides the following subservices:

#### 1. Analysis Subservice (`svc.analysis`)

**Location**: `QMSService.Analysis` (nested class)  
...
**Location**: `QMSService.Structure` (nested class)  
...
**Location**: `QMSService.Calculation` (nested class)  
...
**Location**: `QMSService.Run` (nested class)  
...
**Location**: `QMSService.Project` (nested class)  
...
**Location**: `QMSService.Engine` (nested class)  
...
**Location**: `QMSService.History` (nested class)  
...
The following static methods exist on `QMSService` (marked as "backwards compatibility"):
...
**Exports**:
- `QMSService` (class from `api.service`)
- `get_service(project_root: Optional[Path | str] = None, **kwargs) -> QMSService` (canonical service acquisition)
...
- Compatibility aliases: `QMSServiceError = APIError`, `ErrorSpec = APIError` (placeholder)

**No static methods exported** at top-level. All static methods are accessed via `QMSService.<method>()` directly.
...
| `QMSService.run_calculation()` | `api/service.py:6552` (static) | `main.py:4010` | **VIOLATION** - should use instance method |
| `QMSService.run_step()` | `api/service.py:6649` (static) | `main.py:1753` | **VIOLATION** - should use instance method |
| `QMSService.configure_species_map()` | `api/service.py:6977` (static) | `main.py:3656` | **VIOLATION** - should use instance method |
| `QMSService.generate_kpath()` | `api/service.py:7063` (static) | `main.py:1257` | Acceptable (pure utility) |
| `QMSService.get_default_step_params()` | `api/service.py:7046` (static) | `main.py:1276` | Acceptable (pure utility) |
...
| `QMSService.get_settings()` | `api/service.py:6049` (static) | `server.py:237, 1424, 3046, 4347` | Global operation (acceptable) |
| `QMSService.get_project_summary()` | `api/service.py:6104` (static) | `server.py:1998, 2089` | **VIOLATION** - should use `svc.project.get_summary()` |
| `QMSService.list_structures_data()` | `api/service.py:6167` (static) | `server.py:2010, 2124` | **VIOLATION** - should use `svc.structure.list()` |
| `QMSService.list_calculations_data()` | `api/service.py:6227` (static) | `server.py:2023, 2776` | **VIOLATION** - should use `svc.calculation.list()` |
| `QMSService.init_project()` | `api/service.py:5973` (static) | `server.py:2077` | Acceptable (no project_root yet) |
| `QMSService.import_structure()` | `api/service.py:6779` (static) | `server.py:2116` | **VIOLATION** - should use `svc.structure.import_file()` |
| `QMSService.init_calculation()` | `api/service.py:6310` (static) | `server.py:2766` | **VIOLATION** - should use `svc.calculation.init()` |
| `QMSService.run_calculation()` | `api/service.py:6552` (static) | `server.py:5320` | **VIOLATION** - should use `svc.run.calculation()` |
| `QMSService.run_step()` | `api/service.py:6649` (static) | `server.py:5391` | **VIOLATION** - should use `svc.run.step()` |
| `QMSService.run_single_step()` | `api/service.py:6778` (static) | `server.py:5446` | **VIOLATION** - should use instance method |
| `QMSService.promote_relax_structure()` | `api/service.py:6898` (static) | `server.py:3209` | **VIOLATION** - should use instance method |
| `QMSService.save_relax_final_structure()` | `api/service.py:7206` (static) | `server.py:3600` | **VIOLATION** - should use instance method |
| `QMSService.analyze_project_pseudo_effects()` | `api/service.py:7693` (static) | `server.py:4461` | **VIOLATION** - should use instance method |
| `QMSService.materialize_pseudo_file()` | `api/service.py:7748` (static) | `server.py:4486` | **VIOLATION** - should use instance method |
| `QMSService.get_pseudo_options_for_elements()` | `api/service.py:7775` (static) | `server.py:4576` | **VIOLATION** - should use instance method |
| `QMSService.init_pseudo_dirs()` | `api/service.py:7382` (static) | `server.py:845` | Global operation (acceptable) |
| `QMSService.list_pseudo_libraries()` | `api/service.py:7395` (static) | `server.py:1073` | Global operation (acceptable) |
| `QMSService.get_library_status()` | `api/service.py:7408` (static) | `server.py:1096` | Global operation (acceptable) |
| `QMSService.install_pseudo_library()` | `api/service.py:7424` (static) | `server.py:1135` | Global operation (acceptable) |
| `QMSService.remove_pseudo_library()` | `api/service.py:7471` (static) | `server.py:1164` | Global operation (acceptable) |
| `QMSService.repair_pseudo_library()` | `api/service.py:7488` (static) | `server.py:1187` | Global operation (acceptable) |
| `QMSService.compute_store_size()` | `api/service.py:7504` (static) | `server.py:1198` | Global operation (acceptable) |
| `QMSService.is_pseudo_archive_installed()` | `api/service.py:7516` (static) | `server.py:1331` | Global operation (acceptable) |
| `QMSService.install_pseudo_archive()` | `api/service.py:7532` (static) | `server.py:1354` | Global operation (acceptable) |
| `QMSService.install_sssp_from_seed()` | `api/service.py:7570` (static) | `server.py:889` | Global operation (acceptable) |
| `QMSService.install_all_sssp_from_seed()` | `api/service.py:7593` (static) | `server.py:893` | Global operation (acceptable) |
| `QMSService.download_sssp_library()` | `api/service.py:7612` (static) | `server.py:974` | Global operation (acceptable) |
| `QMSService.download_all_sssp()` | `api/service.py:7646` (static) | `server.py:1018` | Global operation (acceptable) |
| `QMSService.import_seed_archives()` | `api/service.py:7674` (static) | `server.py:1062` | Global operation (acceptable) |
| `QMSService.get_workflow_service()` | `api/service.py:6093` (static) | `server.py:6300, 6334, 6417, 6488` | Acceptable (returns service) |
| `QMSService.create_demo_project()` | `api/service.py:7093` (static) | `server.py:4634` | Acceptable (no project_root yet) |
| `QMSService.list_demo_projects()` | `api/service.py:7167` (static) | `server.py:4657` | Acceptable (no project_root needed) |
| `svc.structure.*` | `api/service.py:1571` (instance) | Not found | Should be used but isn't |
| `svc.calculation.*` | `api/service.py:2119` (instance) | Not found | Should be used but isn't |
| `svc.run.*` | `api/service.py:4322` (instance) | Not found | Should be used but isn't |
...
| `_handle_import_structure` | `QMSService.import_structure(project_root, ...)` | `get_service(project_root).structure.import_file(...)` | ❌ VIOLATION |
| `_handle_init_calculation` | `QMSService.init_calculation(project_root, ...)` | `get_service(project_root).calculation.init(...)` | ❌ VIOLATION |
| `_handle_run_calculation` | `QMSService.run_calculation(project_root, ...)` | `get_service(project_root).run.calculation(...)` | ❌ VIOLATION |
| `_handle_run_step` | `QMSService.run_step(project_root, ...)` | `get_service(project_root).run.step(...)` | ❌ VIOLATION |
| `_handle_promote_relax_structure` | `QMSService.promote_relax_structure(project_root, ...)` | Instance method (TBD subservice) | ❌ VIOLATION |
| `_handle_save_relax_final_structure` | `QMSService.save_relax_final_structure(project_root, ...)` | Instance method (TBD subservice) | ❌ VIOLATION |
| `_handle_analyze_project_pseudo_effects` | `QMSService.analyze_project_pseudo_effects(project_root, ...)` | Instance method (TBD subservice) | ❌ VIOLATION |
| `_handle_materialize_pseudo_file` | `QMSService.materialize_pseudo_file(project_root, ...)` | Instance method (TBD subservice) | ❌ VIOLATION |
| `_handle_get_pseudo_options_for_elements` | `QMSService.get_pseudo_options_for_elements(project_root, ...)` | Instance method (TBD subservice) | ❌ VIOLATION |
...
|---------|----------------------|--------|
| `_handle_init_pseudo_dirs` | `QMSService.init_pseudo_dirs()` | ✅ Acceptable |
| `_handle_list_pseudo_libraries` | `QMSService.list_pseudo_libraries()` | ✅ Acceptable |
| `_handle_get_library_status` | `QMSService.get_library_status(...)` | ✅ Acceptable |
| `_handle_install_pseudo_library` | `QMSService.install_pseudo_library(...)` | ✅ Acceptable |
| `_handle_remove_pseudo_library` | `QMSService.remove_pseudo_library(...)` | ✅ Acceptable |
| `_handle_repair_pseudo_library` | `QMSService.repair_pseudo_library(...)` | ✅ Acceptable |
| `_handle_compute_store_size` | `QMSService.compute_store_size()` | ✅ Acceptable |
| `_handle_get_settings` | `QMSService.get_settings()` | ✅ Acceptable |
| `_handle_init_project` | `QMSService.init_project(...)` | ✅ Acceptable (no project_root yet) |
| `_handle_create_demo_project` | `QMSService.create_demo_project(...)` | ✅ Acceptable (no project_root yet) |
| `_handle_list_demo_projects` | `QMSService.list_demo_projects()` | ✅ Acceptable |
...
1. **Eliminate Static Method Usage for Project-Scoped Operations**
   - **Action**: Migrate all `QMSService.<static_method>(project_root, ...)` calls to `get_service(project_root).<subservice>.<method>(...)`
   - **Files**: `src/qmatsuite/daemon/server.py` (40+ instances), `src/qmatsuite/cli/main.py` (3 instances)
...
3. **Standardize Subservice Method Signatures**
   ...
   - **Files**: `src/qmatsuite/api/service.py`
