...
- **File**: `src/qmatsuite/core/engines/qe_resolver.py`
...
- **Fix**: Added compat re-export: `from qmatsuite.core.paths import home_qe_engines_dir` and included it in `__all__`

### 2. Made `drivers/qe/engine/qe_resolver.py` use re-export for monkeypatching support
- **File**: `src/qmatsuite/drivers/qe/engine/qe_resolver.py`
...
- **File**: `src/qmatsuite/core/engines/qe_diagnostics.py` (new file)
- **Change**: Created backward-compatibility re-export for `qe_diagnostics` functions
- **Reason**: Test was importing from `qmatsuite.core.engines.qe_diagnostics` but module was moved
...
- **File**: `src/qmatsuite/core/engines/qe_binary_locator.py` (new file)
- **Change**: Created backward-compatibility re-export for `qe_binary_locator` functions
- **Reason**: Test was importing `locate_pw2wannier90` from `qmatsuite.core.engines.qe_binary_locator` but module was moved
...
- **Change**: Changed import from `from qmatsuite.core.engines.qe import EngineConfig` to `from qmatsuite.core.engines.base import EngineConfig`
...
1. `src/qmatsuite/core/engines/qe_resolver.py` - Added `home_qe_engines_dir` re-export
2. `src/qmatsuite/drivers/qe/engine/qe_resolver.py` - Modified to support monkeypatching via re-export
3. `src/qmatsuite/core/engines/qe_diagnostics.py` - Created (new file)
4. `src/qmatsuite/core/engines/qe_binary_locator.py` - Created (new file)
