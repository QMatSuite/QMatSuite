...
**Key ownership / key access enforcement**: The rule that each parameter key (section.key) belongs to exactly one ParamSpace. Enforcement prevents one ParamSpace from accessing keys owned by another, ensuring lifecycle-wide isolation. Violations raise `KeyAccessError` (defined in `src/qmatsuite/presets/paramspace.py:132`).
...
- Location: `src/qmatsuite/presets/paramspace.py:164-255`
...
**Implementation**: `src/qmatsuite/presets/integration.py:apply_presets_to_step()` (line ~450-650) implements two-phase compilation:
...
**Oracle concept**:
- **What it is**: Read-only semantic prerequisite queries (`src/qmatsuite/presets/oracle.py:Oracle`).
...
- Phase ordering is enforced by `src/qmatsuite/presets/integration.py:apply_presets_to_step()` (line ~450-650).
...
- Oracle implementation: `src/qmatsuite/presets/oracle.py:Oracle` (line 18-60).
- Oracle usage: `src/qmatsuite/presets/integration.py:apply_presets_to_step()` (line ~622-629) creates oracle and passes it to `apply_invariants()`.
...
- `src/qmatsuite/presets/variants_registry.py`: Variant lookup and compilation boundaries
- `src/qmatsuite/presets/integration.py`: Integration layer that applies patches to `StepDoc`
...
**Implementation**:
- IR canonical boolean encoder: `src/qmatsuite/ir/backends/qe/mapping.py:ir_bool()` converts `True`/`False` → `.true.`/`.false.`
- QE writer: `src/qmatsuite/ir/backends/qe/mapping.py:ir_to_qe_param()` handles type conversion for QE-specific keys
- ParamSpace compile: `src/qmatsuite/presets/paramspace.py:compile_profile_patch()` writes IR-native types
- Integration layer: `src/qmatsuite/presets/integration.py:apply_presets_to_step()` applies IR patches, then QE writer converts to engine format
...
**Implementation**: 
- Match: `src/qmatsuite/presets/paramspace.py:match_profile()` (line ~484-560)
- Compile: `src/qmatsuite/presets/paramspace.py:compile_profile_patch()` (line ~650-850)
...
**Detection**: Project run is detected when `calculation_dir and project_root and calc_model is not None` (see `src/qmatsuite/calculation/structure_steps.py:1134`).

**Enforcement location**: `src/qmatsuite/calculation/structure_steps.py:1147-1187`
...
**Data model location**: `src/qmatsuite/core/models.py:CalculationModel.species_map` (line ~243)
...
**Exact warning string** (from `src/qmatsuite/calculation/structure_steps.py:1139-1142`):
```
...
- System MUST instruct users to use `qms configure species ...`

**Exact warning string** (from `src/qmatsuite/cli/main.py:1193-1195`):
```
...
**Spec note**: Step-level `--SPECIES.*` is legacy; allowed but warns; preferred is `qms configure species --from-input <file.in>` or `qms configure species --set "ELEMENT:MASS:PSEUDOPOT"`.

### 4.5 Shared API (single implementation)

**Shared function location**: `src/qmatsuite/calculation/species_config.py:configure_species_map()` (line 22-124)

**Consumers**:
- CLI command: `src/qmatsuite/cli/main.py:configure_species_command()` (line ~3079-3178) calls `configure_species_map()` at line 3159
- QMSService method: `src/qmatsuite/api.py:QMSService.configure_species_map()` (line 7984-8030) calls `configure_species_map()` at line 8016

**Contract**:
- Both CLI and QMSService MUST call the shared API (`configure_species_map()`).
- Neither CLI nor QMSService MAY write `calculation.yaml` directly in their own logic.
...
**CLI workflow**:
- Configure `species_map` via `qms configure species --from-input <qe_input.in>` (extracts from ATOMIC_SPECIES)
- Or use `qms configure species --set "ELEMENT:MASS:PSEUDOPOT"` for explicit entries
- Step-level `--SPECIES.*` flags are legacy and ignored in project runs (warn only)

**Daemon/GUI workflow**:
- Configure `species_map` via `QMSService.configure_species_map(project_root, calculation, from_qe_input=..., set_entries=...)`
- Both CLI and daemon/GUI use the shared API (`src/qmatsuite/calculation/species_config.py:configure_species_map()`)
...
- Missing `species_map` → hard error (`ValueError`) with message:
  ...
  ```
  (from `src/qmatsuite/calculation/structure_steps.py:1152-1158`)
  **Spec note**: Error message mentions `--SPECIES.*` as legacy wording; preferred path is `qms configure species --from-input` or `qms configure species --set`.
...
- Project run entrypoints (e.g., `QMSService.run_calculation()`, `CalculationRunner.run()`) MUST provide `calc_model` (or equivalent calc context) and `project_root`.
...
- Standalone entrypoints (e.g., `run_standalone_step()`, CLI `qms run step --standalone`) MUST NOT be used for normal project `step.yaml` files.
...
- Project run detection: `src/qmatsuite/calculation/structure_steps.py:1134` (`is_project_run = (calculation_dir and project_root and calc_model is not None)`)
- Project run entrypoints: `src/qmatsuite/api.py:QMSService.run_calculation()` (line ~1151), `src/qmatsuite/calculation/runner.py:CalculationRunner.run()` (line ~67)
- Standalone entrypoints: `src/qmatsuite/cli/main.py:_run_standalone_step()` (line ~1700+)
...
**Location**: `src/qmatsuite/calculation/structure_steps.py:1193-1199` sets `allow_step_species_overrides=not is_project_run` (project runs: False, standalone: True)
...
**Location**: 
- Executor: `src/qmatsuite/calculation/compat_executor.py:run_qe_step_from_existing_input_compat()` (line 22-285)
- Runner integration: `src/qmatsuite/calculation/runner.py:CalculationRunner.run()` accepts `compat_input_playback: bool = False` (line 140)
- Handler: `src/qmatsuite/execution/handlers.py:qe_step_handler()` checks `compat_input_playback` flag (line ~190-200)
...
- [ ] No new code path writes `species_map` except shared API (`src/qmatsuite/calculation/species_config.py:configure_species_map()`)
...
- [ ] Compile phases/oracle behavior unchanged (two-phase compilation in `src/qmatsuite/presets/integration.py:apply_presets_to_step()`)
...
- [ ] Daemon tests MUST NOT spawn the CLI as a subprocess for species configuration; they must use `QMSService.configure_species_map()` API
- [ ] CLI tests MUST NOT patch `calculation.yaml` via direct YAML editing; they must use `qms configure species ...` where applicable
...
- **Q1 (Verification)**: In standalone mode, what happens if a `step.yaml` belongs to a calculation (has `parent_calculation_id`) but is run standalone? Current code at `src/qmatsuite/calculation/structure_steps.py:1134` detects project run if `calculation_dir and project_root and calc_model is not None`, but standalone mode may not set these. **Verify**: Check `src/qmatsuite/cli/main.py:_run_standalone_step()` and `src/qmatsuite/calculation/structure_steps.py:1134-1199` to confirm error behavior when standalone is invoked on a project step.
- **Q2 (Verification)**: Are there any edge cases where `species_map` is partially populated (some elements missing) but not detected as incomplete? Current validation at `src/qmatsuite/calculation/structure_steps.py:1160-1187` checks for missing elements and placeholders. **Verify**: Review validation logic at lines 1160-1187 to confirm all edge cases are covered (empty dict, None values, missing keys, placeholder detection).
- **Q3 (Verification)**: The shared API `configure_species_map()` writes directly to `calculation.yaml` without explicit locking. Is this safe for concurrent access? Current implementation at `src/qmatsuite/calculation/species_config.py:122` uses `write_text()` without lock. **Verify**: Check if `calculation.yaml` writes are protected by edit locks elsewhere (e.g., `calc_edit_lock` in `src/qmatsuite/core/locking.py`) or if concurrent access is a risk.
