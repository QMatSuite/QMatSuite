...
**IR Definition**: `src/qmatsuite/presets/paramspace.py:ParamKey` docstring (lines 64-71)
...
**IR Canonical Encoder**: `src/qmatsuite/ir/backends/qe/mapping.py:ir_bool()` (lines 14-41)
...
**IR Mapping**: `src/qmatsuite/ir/backends/qe/mapping.py:IR_TO_QE_MAPPING` (line 46-74)
...
**Evidence**: `src/qmatsuite/presets/integration.py:apply_presets_to_step()` (lines 712-720)
```python
...
**Evidence**: `src/qmatsuite/ir/backends/qe/mapping.py:ir_params_to_qe_params()` (lines 218-278)
...
**Evidence**: `src/qmatsuite/calculation/structure_steps.py:StructureStepSpec.from_yaml()` (lines 150-169)
...
**Evidence**: `src/qmatsuite/calculation/structure_steps.py:generate_qe_input_from_spec()` (line 487-524)
...
**Evidence**: `src/qmatsuite/io/generator.py:QEInputGenerator.write_file()`
...
**Where defined**: `src/qmatsuite/workflow/registry.py:StepTypeSpec` (lines 23-58)
...
- Evidence: `src/qmatsuite/workflow/step_factory.py:73` - `"step_type": machine_step_type`

**StepType Enum**:

**Where defined**: `src/qmatsuite/calculation/types.py:StepType` (lines 10-30)
...
|---|---|---|
| `src/qmatsuite/presets/variants_registry.py:get_variant()` (lines 257-265) | Preset/ParamSpace step selection | **YES** - Determines which variants apply to step |
| `src/qmatsuite/api.py:normalize_step_type_to_public()` (line 3438-3439) | UI display (output file extensions) | **NO** - Display/naming only |
| `src/qmatsuite/api.py:get_step_detail()` (line 3437) | File naming (output file extensions) | **NO** - File naming only |
| `src/qmatsuite/workflow/registry.py:StepTypeRegistry.list_all()` (line 624-626) | API listing (returns public types) | **NO** - API response only |
| `src/qmatsuite/workflow/registry.py:StepTypeRegistry.list_accepting_presets()` (line 646-651) | Preset catalog (returns public types) | **NO** - Catalog only |
...
|---|---|---|
| `src/qmatsuite/workflow/step_factory.py:create_step_doc()` (line 73) | step.yaml storage (`step_type` field) | **YES** - Persistent storage |
| `src/qmatsuite/calculation/step.py:Step.run()` (line 92) | Runner execution (reads from step.yaml) | **YES** - Determines executable/binary |
| `src/qmatsuite/engine/qe_engine.py:QeEngine.run_step()` (lines 40-41) | Engine execution (step_type_value passed to backend) | **YES** - Determines QE executable |
| `src/qmatsuite/engine/pyscf_engine.py:PySCFEngine.run_step_with_chain()` (lines 474-483) | PySCF execution (reads machine_type from step.yaml) | **YES** - Determines PySCF step type |
| `src/qmatsuite/calculation/structure_steps.py:materialize_step_spec()` (lines 747-772) | Materialization routing (Wannier90/PySCF/ORCA detection) | **YES** - Determines input generator |
...
1. **`src/qmatsuite/calculation/step.py:29`** - `step_type: Optional[StepType] = None`
...
2. **`src/qmatsuite/calculation/runner.py:50-77`** - `_coerce_step_type()`
   - **Classification**: Legacy coercion only (acceptable)
   - **Logic influence**: Converts string to Enum for backward compatibility, but execution uses string from step.yaml

3. **`src/qmatsuite/api.py:7756-7758`** - Validation: `if step_type not in (StepType.RELAX.value, StepType.VC_RELAX.value)`
...
4. **`src/qmatsuite/api.py:7866-7868`** - Same as above (duplicate validation)
   - **Classification**: **Core logic usage (NOT acceptable; SSOT violation)**
   - **Logic influence**: Same as above

5. **`src/qmatsuite/api.py:1292`** - API response: `"step_type": s.step_type.value if hasattr(s.step_type, 'value') else str(s.step_type)`
   - **Classification**: Legacy coercion only (acceptable)
   - **Logic influence**: API response formatting only

6. **`src/qmatsuite/api.py:2376`** - API response: Same as above
   - **Classification**: Legacy coercion only (acceptable)
   - **Logic influence**: API response formatting only

7. **`src/qmatsuite/cli/main.py:1746`** - Standalone step creation: `step_type=StepType.from_string(spec.step_type)`
   - **Classification**: Legacy coercion only (acceptable)
   - **Logic influence**: Creates Step object with Enum, but execution reads from step.yaml

**Core Logic Violations**:

- **`src/qmatsuite/api.py:7756-7758`** and **`7866-7868`**: Enum used for **validation logic** in preset application. This is a **SSOT violation** because:
...
1. **`src/qmatsuite/workflow/registry.py:StepTypeRegistry.get()`** (lines 594-618)
...
2. **`src/qmatsuite/presets/variants_registry.py:get_variant()`** (lines 257-265)
...
3. **`src/qmatsuite/workflow/generalized_steps.py:materialize_public_step_key()`** (lines 161-212)
...
**Evidence**:
- `src/qmatsuite/presets/paramspace.py:601-605` converts Python `bool` to `.true.`/`.false.` strings
- `src/qmatsuite/presets/integration.py:718-719` ensures QE format before applying to StepDoc
...
**Evidence**:
- `src/qmatsuite/presets/paramspace.py:601-605` calls `ir_bool()` to convert Python `bool` to `.true.`/`.false.` strings
...
**Evidence**:
- `src/qmatsuite/calculation/step.py:Step.run()` reads `step_type_value = self.step_type.value` (line 92), but this is from `Step.step_type` Enum (legacy)
- **Actual execution**: `engine.backend.run_step()` receives `step_type_value` (line 113), but engine reads from step.yaml directly
- `src/qmatsuite/engine/pyscf_engine.py:474-483` explicitly reads `machine_type` from step.yaml (not from `Step.step_type` Enum)
- `src/qmatsuite/calculation/structure_steps.py:materialize_step_spec()` uses `machine_type` for routing (lines 747-772)
...
1. **`src/qmatsuite/api.py:7756-7758`** - Validation for relax operations
   - **Impact**: Blocks ORCA/PySCF steps from using relax operations
   - **Fix**: Replace with `StepTypeRegistry` lookup

2. **`src/qmatsuite/api.py:7866-7868`** - Same validation (duplicate)
...
- **StepType Enum still exists**: `src/qmatsuite/calculation/types.py:StepType` (Enum) coexists with `StepTypeRegistry` (string-based). Some code paths may still use Enum.
...
1. **Key ownership uniqueness**: Each `(section, key)` belongs to exactly one ParamSpace. Enforced by `register_paramspace()` and `check_key_access()` (`src/qmatsuite/presets/paramspace.py:168-254`).
...
3. **species_map SSOT**: Project runs require `calculation.yaml` `species_map`; step-level `species_overrides` are ignored. Enforced by `src/qmatsuite/calculation/structure_steps.py:1134-1187`.
4. **Compile phase ordering**: Prerequisite dimensions (occupations_scheme, magnetism) compile before dependent ones (precision). Enforced by `src/qmatsuite/presets/integration.py:414-576`.
5. **Type formatting boundary**: ParamSpace writes IR-native types; `ir_bool()` converts to canonical strings. Enforced by `src/qmatsuite/presets/paramspace.py:601-605` and `src/qmatsuite/ir/backends/qe/mapping.py:14-41`.
...
**Evidence**:
- **Key ownership registry**: `src/qmatsuite/presets/paramspace.py:162-205`
...
- **Reversibility**: `src/qmatsuite/presets/paramspace.py:461-534` (match), `541-612` (compile)
...
- **Multi-phase compilation**: `src/qmatsuite/presets/integration.py:414-576`
...
**Evidence**:
- **Match logic**: `src/qmatsuite/presets/paramspace.py:461-534`
...
- **Custom detection**: `src/qmatsuite/presets/variants_registry.py:499-502`
...
**Evidence**:
- **Detection**: `src/qmatsuite/calculation/structure_steps.py:1134`
...
- **Shared API**: `src/qmatsuite/calculation/species_config.py:configure_species_map()` (line 22-124)
  - Used by CLI (`src/qmatsuite/cli/main.py:3159`) and QMSService (`src/qmatsuite/api.py:8016`)
...
**Evidence**:
- **Project run**: `src/qmatsuite/calculation/structure_steps.py:materialize_step_spec()` (line 671-1797)
...
- **Standalone**: `src/qmatsuite/cli/main.py:_run_standalone_step()` (line ~1700+)
  - Import `.in` → temporary `step.yaml` → run using production pipeline (roundtrip)
- **Compat playback**: `src/qmatsuite/calculation/compat_executor.py:run_qe_step_from_existing_input_compat()` (line 22-285)
...
**Evidence**:
- **Materialization**: `src/qmatsuite/calculation/structure_steps.py:1188-1200`
  - `set_outdir_to_temp()` and `set_pseudo_dir_in_input()` called during `materialize_step_spec()`
- **Pseudo staging**: `src/qmatsuite/core/pseudo.py:ensure_qe_pseudos()` (line 171-301)
...
**Evidence**:
- **ParamSpace compile**: `src/qmatsuite/presets/paramspace.py:601-605`
  - Converts Python `bool` to IR canonical string using `ir_bool()` before writing patch
- **IR canonical encoder**: `src/qmatsuite/ir/backends/qe/mapping.py:14-41`
  - `ir_bool()` converts `True`/`False` → `.true.`/`.false.`
- **QE writer**: `src/qmatsuite/ir/backends/qe/mapping.py:ir_to_qe_param()` (line 92-124)
...
**ParamSpace**: `src/qmatsuite/presets/paramspace.py:298-372`
...
**Dimension**: String identifier (e.g., `"precision"`, `"magnetism"`). Defined in `src/qmatsuite/presets/dimensions.py`.
...
**Key**: `src/qmatsuite/presets/paramspace.py:ParamKey` (line 59-121)
...
**Ownership Registry**: `src/qmatsuite/presets/paramspace.py:162-205`
...
**Preset**: User-facing option (e.g., `"high"`, `"medium"`, `"low"` for precision). Mapped to profile name via `ENUM_TO_PROFILE` (`src/qmatsuite/presets/variants_registry.py:233-238`).

**Profile**: Named set of cells in a ParamSpace (e.g., `"LOW"`, `"MED"`, `"HIGH"`). Defined in `paramspace.profiles` dict.

**ParamSpace Compiler Phase**: `src/qmatsuite/presets/integration.py:414-576`
- Phase 1 (prerequisite): `occupations_scheme`, `magnetism` (lines 437-472)
- Phase 2 (dependent): `precision`, `convergence` (lines 490-575)
- Ordering is hardcoded in `apply_presets_to_step()`

**Oracle**: `src/qmatsuite/presets/oracle.py:Oracle` (line 18-60)
- Read-only semantic prerequisite queries
- Example: `oracle.degauss_applicability()` returns `True` iff `SYSTEM.occupations == "smearing"`
- Created at `src/qmatsuite/presets/integration.py:622`, passed to `apply_invariants()` at line 629
...
**QE Parameter Catalog**: `src/qmatsuite/data/qe_module_parameters.json`
- Schema v3: `modules → parameters` map
- Each parameter has: `namelist`, `name`, `type`, `default`, `enum`, `description`
- Used by UI (`src/qmatsuite/api.py:list_qe_ui_parameters`) and validation
...
**StepType Enum**: `src/qmatsuite/calculation/types.py:StepType` (line 10-30)
- Legacy Enum with values like `StepType.SCF = "scf"`
- Still used in some code paths (e.g., `src/qmatsuite/calculation/step.py:29`)

**StepTypeRegistry**: `src/qmatsuite/workflow/registry.py:StepTypeRegistry` (line 566-640)
...
**Where defined**:
- **Dimension**: String constant in `src/qmatsuite/presets/dimensions.py` (e.g., `DIMENSION_PRECISION = "precision"`)
- **Variant**: `src/qmatsuite/presets/space_variant.py:ParamSpaceVariant` (line 16-51)
- **Key**: `src/qmatsuite/presets/paramspace.py:ParamKey` (line 59-121)
- **ParamSpace**: `src/qmatsuite/presets/paramspace.py:ParamSpace` (line 298-372)

**How variant selection works**:
- `src/qmatsuite/presets/variants_registry.py:get_variant()` (line 245-268)
...
- `src/qmatsuite/presets/integration.py:apply_presets_to_step()` (line 328-997)
...
- Interface: `src/qmatsuite/presets/oracle.py:Oracle` (line 18-60)
...
**Entrypoint(s)**:
- CLI: `src/qmatsuite/cli/main.py:configure_presets_command()` (calls `apply_presets_to_step()`)
- QMSService: `src/qmatsuite/api.py:QMSService.apply_presets_to_step()` (calls `apply_presets_to_step()`)
- Direct: `src/qmatsuite/presets/integration.py:apply_presets_to_step()` (line 328)

**Where patches are computed**:
- `src/qmatsuite/presets/variants_registry.py:compile_dimension_patch_for_step()` (line 271-335)
  - Calls `compile_profile_patch()` (line 330) within `ParamSpaceContext` (line 329)
- `src/qmatsuite/presets/paramspace.py:compile_profile_patch()` (line 541-612)
...
**What "match" means**:
- `src/qmatsuite/presets/paramspace.py:match_profile()` (line 461-534)
...
- `src/qmatsuite/presets/variants_registry.py:detect_dimension_for_step()` (line 485-520)
...
- `src/qmatsuite/ir/backends/qe/mapping.py:IR_TO_QE_MAPPING` (line 46-74)
  - Maps IR keys to `(qe_module, qe_section, qe_key)` tuples
- `src/qmatsuite/ir/backends/qe/mapping.py:ir_to_qe_param()` (line 92-124)
...
- **Production**: `tools/extract_qe_parameters_v2.py` scrapes QE HTML docs, generates `src/qmatsuite/data/qe_module_parameters.json` (schema v3)
- **Consumption**: `src/qmatsuite/data/qe_metadata.py` loads JSON, provides `get_module_param_sections()` API
- **UI**: `src/qmatsuite/api.py:list_qe_ui_parameters()` (line 692-709) returns parameters for UI display
...
1. **step.yaml + calculation.yaml + project resources**:
   - `step.yaml` loaded via `StructureStepSpec.from_yaml()` (`src/qmatsuite/calculation/structure_steps.py:487-665`)
...
- **Evidence**: `src/qmatsuite/io/generator.py:QEInputGenerator.write_file()` writes fresh `.in` file from `QEInput` object. No parsing of existing `.in` files during production run.

**Confirm runtime-managed keys override behavior**:
- **Evidence**: `src/qmatsuite/calculation/structure_steps.py:1188-1200`
...
- **Only required UPFs staged**: `src/qmatsuite/core/pseudo.py:ensure_qe_pseudos()` (line 171-301)
  - Copies files from `system_pseudo_dir` to `project_pseudo_dir` based on `species_map` entries
- **pseudo_dir points to project/pseudo**: `src/qmatsuite/calculation/structure_steps.py:1188-1200`
  - `set_pseudo_dir_in_input()` sets `CONTROL.pseudo_dir` to `project_root/pseudo`
- **calc species_map authoritative**: `src/qmatsuite/calculation/structure_steps.py:1134-1187`
...
- **Evidence**: `src/qmatsuite/calculation/structure_steps.py:1139-1143`
...
- `src/qmatsuite/calculation/types.py:StepType` (line 10-30)
  - Values: `SCF = "scf"`, `NSCF = "nscf"`, `DOS = "dos"`, `BANDS_PW = "bands_pw"`, `BANDS = "bands"`, `PH = "ph"`, `Q2R = "q2r"`, `MATDYN = "matdyn"`, `DYNMAT = "dynmat"`, `PP = "pp"`, `PROJWFC = "projwfc"`, `RELAX = "relax"`, `VC_RELAX = "vc-relax"`, `W90_PREPROC = "w90_preproc"`, `PW2WANNIER90 = "pw2wannier90"`, `W90_RUN = "w90_run"`, `PYSCF_SCF = "pyscf_scf"`, `CUSTOM = "custom"`

**public_type / gen step strings**:
- Defined in `src/qmatsuite/workflow/registry.py:StepTypeSpec` (line 23-58)
...
**In UI**:
- `src/qmatsuite/api.py:list_workflow_templates()` (line ~3437) uses `normalize_step_type_to_public()` to convert machine_type to public_type for display
...
- `src/qmatsuite/calculation/structure_steps.py:materialize_step_spec()` (line 671-1797)
...
- `src/qmatsuite/engine/qe_engine.py:QeEngine.run_step()` uses `step_type` to select executable (e.g., `pw.x` for `qe_scf`, `bands.x` for `qe_bands`)

### 8.4 Mapping Logic: Gen/Public ↔ Machine/Spec

**What module performs mapping**:
- `src/qmatsuite/workflow/registry.py:StepTypeRegistry` (line 566-640)
...
- `src/qmatsuite/calculation/step.py:29` - `step_type: Optional[StepType] = None`
- `src/qmatsuite/calculation/runner.py:50-77` - `_coerce_step_type()` converts string to Enum
...
1. **StepType Enum coexistence with StepTypeRegistry**
   ...
   - **Evidence**: `src/qmatsuite/calculation/step.py:29`, `src/qmatsuite/calculation/runner.py:50-77`
...
3. **QE parameter catalog and presets can drift**
   ...
   - **Evidence**: Catalog in `src/qmatsuite/data/qe_module_parameters.json`, presets in `src/qmatsuite/presets/variants_registry.py`
...
4. **Variant definitions use hardcoded public_type strings**
   ...
   - **Evidence**: `src/qmatsuite/presets/variants_registry.py:48-100` (variant definitions)
...
5. **Runtime-managed keys scattered across codebase**
   ...
   - **Evidence**: `src/qmatsuite/calculation/structure_steps.py:1188-1200`, `src/qmatsuite/core/pseudo.py:ensure_qe_pseudos()`
...
7. **Compat playback mode may leak into production**
   ...
   - **Evidence**: `src/qmatsuite/calculation/runner.py:140` (flag definition), `src/qmatsuite/execution/handlers.py:190-200` (flag check)
...
8. **Step-level species_overrides warning may be ignored**
   ...
   - **Evidence**: `src/qmatsuite/calculation/structure_steps.py:1139-1143` (warning only, not error)
...
9. **Oracle interface is minimal (only degauss_applicability)**
   ...
   - **Evidence**: `src/qmatsuite/presets/oracle.py:Oracle` (line 18-60) - only one method
...
10. **Type formatting boundary may be violated**
    ...
    - **Evidence**: `src/qmatsuite/presets/paramspace.py:601-605` (converts bool to string), but other code paths may bypass
...
11. **Materialization path for non-QE engines unclear**
    ...
    - **Evidence**: `src/qmatsuite/calculation/structure_steps.py:777-850` (Wannier90 path), `767-772` (PySCF/ORCA detection)
...
12. **StepTypeRegistry lookup may fail silently**
    ...
    - **Evidence**: `src/qmatsuite/presets/variants_registry.py:262-265` (returns `None` if spec not found)
...
- **IR→engine params mapping responsibility**: Create `src/qmatsuite/ir/backends/orca/mapping.py` (or `pyscf/mapping.py`) with `IR_TO_ORCA_MAPPING` (or `IR_TO_PYSCF_MAPPING`) dict
...
- **StepTypeRegistry entries**: Add ORCA/PySCF step types to `_STEP_TYPES` dict in `src/qmatsuite/workflow/registry.py`
...
- **Registries**: `src/qmatsuite/workflow/registry.py:StepTypeRegistry` - add step type specs
- **Resolvers**: `src/qmatsuite/workflow/generalized_steps.py:materialize_public_step_key()` - add engine family mapping
- **Materializers**: `src/qmatsuite/calculation/structure_steps.py:materialize_step_spec()` - add engine-specific input generation path (similar to Wannier90 path at lines 777-850)
- **IR mapping**: `src/qmatsuite/ir/backends/{engine}/mapping.py` - create engine-specific mapping module
...
```bash
rg -n "class Dimension|class Variant|owned_keys|KeyAccess|KeyAccessError|oracle|apply_invariants|compile_" -S src/qmatsuite/presets
rg -n "public_type|machine_type|StepType|step_type_raw|resolve_.*step|StepTypeRegistry" -S src/qmatsuite
rg -n "species_map|species_overrides|configure_species_map|pseudo_dir|ensure_qe_pseudos" -S src/qmatsuite
rg -n "qe_parameter.*json|active|common|advanced|freeform" -S src/qmatsuite
